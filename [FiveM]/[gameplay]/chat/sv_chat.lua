...
Conditions = {
    ...
    ["JOB"] = function(src, params, infos)
        ...
        local currJob = user:getVar("job")
        if params then
            if params == currJob then
                return true
            else
                TriggerClientEvent("chatMessage", src, "SYSTEM ", {255, 0, 0}, "You don't have access to this command")
            end
        else
            return false
        end
    ...
    ["ADMIN"] = function(src, params, infos)
        local user = exports["ev-base"]:getModule("Player"):GetUser(src)
        local userRank = exports["ev-lib"]:getUserRank(src)
...
RegisterNetEvent("ev-commands:buildCommands")
AddEventHandler("ev-commands:buildCommands", function()
    local _src = source
    if _src then src = _src end
    
    local user = exports["ev-base"]:getModule("Player"):GetUser(_src)
    local suggestions = {}
    local infos = {
        ["rank"] = user:getVar("rank"),
        ["job"] = user:getVar("job"),
...
local function refreshCommands(player)
    local user = exports["ev-base"]:getModule("Player"):GetUser(player)
    local suggestions = {}
    local infos = {
        ["rank"] = user:getVar("rank"),
        ["job"] = user:getVar("job"),
...
RegisterServerEvent('ev-admin:server:SendReport')
AddEventHandler('ev-admin:server:SendReport', function(name, targetSrc, msg)
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local userRank = exports["ev-lib"]:getUserRank(src)
    local defaultPower = exports["ev-admin"]:getRank("user");
    ...
    if (userPower > defaultPower) then
        ...
        TriggerEvent("ev-log:server:CreateLog", "AdminReport", "Report", "green", "Report - ( Name  " .. name .. ' ) ( Id : ' .. targetSrc .. ' ) ( Msg : ' .. msg .. ' ) ')
    end
end)

RegisterServerEvent('ev-admin:server:StaffChatMessage')
AddEventHandler('ev-admin:server:StaffChatMessage', function(name, msg)
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local userRank = exports["ev-lib"]:getUserRank(src)
    local defaultPower = exports["ev-admin"]:getRank("user");
    ...
    if (userPower > defaultPower) then
        ...
        TriggerEvent("ev-log:server:CreateLog", "AdminChat", "Staff Chat", "green", "Staff - " .. name .. " message: " ..  msg)
    end
end)
