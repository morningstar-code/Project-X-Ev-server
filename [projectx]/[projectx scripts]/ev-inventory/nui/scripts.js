...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0, targetInvSlots = 40) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
    ...
    } else if (targetinvName.indexOf('trays_rameesanct') > -1) {
        secondaryMaxWeight = 300.0;
        slotLimitTarget = 5;
        ...
        } else if (targetinvName.indexOf('pawnhub_counter') > -1) {
            secondaryMaxWeight = 350.0;
            displayName = 'Counter';
        }

    } else if (targetinvName.startsWith('paynless-') || targetinvName.startsWith('rr_hotel-') || targetinvName.startsWith('casino_hotel-') || targetinvName.startsWith('roosterretreat-') || targetinvName.startsWith('owo_storage-')) {
        slotLimitTarget = 40;
        ...
        if (occupiedslot && !isDragging) {
            ...
            if (moveAmount != 0 && $('#draggedItem > img')) {
                ...
                if( $('#draggedItem > img').data('weight') != null) {
                    ...
                    result ="Could not find weight of item";
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            if (itemList[itemid].weapon) {
                $.post('https://ev-inventory/weaponCrafted', JSON.stringify({
                  "item": itemid,
                }));
              }
...
function AttemptDropInFilledSlot(slot) {
    ...
    if (itemid1 != itemid2 && TargetInventoryName.includes('player-Shop')){
        result2 = '';
        result = "Cannot stack onto a different item from a player store.";
    }
                    ...
                    if (purchaseCost > userCash && purchaseCost !== 0) {
                        ...
                        if (itemList[itemidsent].weapon && inventoryReturnItemDropName !== inventoryDropName) {
                            if (!exluded[itemidsent] && !userWeaponLicense) {
                                result = "You don't have a license!";
                                result2 = "You don't have a license!";
                                InventoryLog('Error: ' + result);
                                EndDragError(slot);
                                return;
                            }
...
function setPriceForItem(item,targetName, itemData) {
    if(priceEdit == ''){
        ...
        if(price >= 1){
            ...
            $.post('https://ev-inventory/invSetPrice', JSON.stringify(information));
