...
on('watch-inventory', (src, cash, name) => {
    watch = [src, name]
    emit('chatMessage', 'SEARCH ', 2, "Player had cash in the amount of: " + cash);
});
...
const getWeaponsLicense = Cacheable(async (ctx, cid) => [true, await RPC.execute("ev-licenses:hasLicense", "Weapon", cid)], { timeToLive: 1 * 5 * 1000 })
const getClass2WeaponsLicense = Cacheable(async (ctx, cid) => [true, await RPC.execute("ev-licenses:hasLicense", "WeaponClass2", cid)], { timeToLive: 1 * 5 * 1000 })

let isCuffed = false;
RegisterNetEvent('ev-police:cuffs:state');
on('ev-police:cuffs:state', (pIsHandcuffed, pIsHandcuffedAndWalking) => {
...
on('inventory-open-request', async () => {
    ...
    if (isInVehicle) {
        ...
        if (!IsThisModelABicycle(vehicleModel) && !IsThisModelABike(vehicleModel)) {
            ...
            const vehId = exports['ev-vehicles'].GetVehicleIdentifier(vehicleFound)
            if (!vehId) {
        ...
        let serverCode = 'wl' //exports["ev-config"].GetServerCode();
...
function openTrunk(pEntity) {
    ...
    const vehId = exports['ev-vehicles'].GetVehicleIdentifier(pEntity)
    if (!vehId) {
        CloseGui();
...
on('ev-inventory:openDumpster', (pParams, pEntity, pContext) => {
    openDumpster(pEntity);
});

on('ev-inventory:openTrunk', (pParams, pEntity, pContext) => {
    openTrunk(pEntity);
});
...
on('__cfx_nui:weaponCrafted', (data, cb) => {
    cb("ok");
    emitNet('ev-inventory:weaponCrafted', data.item);
});
    ...
    if (canOpen === true) {
        ...
        if (targetinvName.indexOf("Shop") > -1) {
            ...
            setImmediate(async () => {
                ...
                SendNuiMessage(JSON.stringify({ response: 'cashUpdate', amount: cash, weaponlicence: hasWeapons, class2license: hasClass2Weapons, brought: brought, cop: cop }));
            })
        }
...
function hasPermissionInProperty(){
    if(exports["ev-housing"].isInsideProperty() && exports["ev-housing"].hasPermissionInProperty() == true && exports["ev-housing"].isPropertyShop())
        return true
    else
        return false 
}
...
function OpenJewelryInventory() {
    ...
    emit("inventory-open-container", inventory, 8, 30.0);
}
...
function OpenProtectedInventory() {
    ...
    emit("inventory-open-container", inventory, 8, 70.0);
}
...
const doTranslations = async () => {
    let isReady = exports['ev-i18n'].IsReady();
    while (!isReady) {
        await Delay(1000);
        isReady = exports['ev-i18n'].IsReady();
    }
    for (const key of Object.keys(itemList)) {
        ...
        itemList[key].displayname = 
            exports['ev-i18n'].GetStringSwap(itemList[key].__og_displayname || itemList[key].displayname);
        itemList[key].information = 
            exports['ev-i18n'].GetStringSwap(itemList[key].__og_information || itemList[key].information);
        if (itemList[key].foodCategory) {
            itemList[key].foodCategory = (itemList[key].__og_food_cats || itemList[key].foodCategory)
                .map(c => exports['ev-i18n'].GetStringSwap(c));
        }
    }
...
on('ev-i18n:languageChanged', doTranslations);
...
on('ev-inventory:itemUsed', async (pItem, pInfo, pInv, pSlot) => { 
    ...
    if (!meta.id) {
        ...
        getStuff = await exports['ev-inventory'].getItemInformation(pSlot);
        meta = JSON.parse(getStuff)
        /* meta2 = {"id":1391536,"copper":500,"_hideKeys":["copper"],"Copper":500}; */
        /* console.log(JSON.parse(meta));  */
        /* console.log(meta2); */
        console.log("Data got"); 
        /* meta = await RPC.execute('ev-inventory:getItemInformation', pInv, pSlot); */ 
    }
    ...
    const prompt = await exports['ev-ui'].OpenInputMenu( 
        [
            {
                label: 'Action',
                name: 'action',
    ...
    if (prompt.action === 'add') {
        for (const key of validKeys) {
            const qty = Math.min(exports['ev-inventory'].getQuantity(key, true), 50000); 
            if (!qty) {
                continue;
            }
            if (!meta[key]) {
        ...
        /* TriggerServerEvent('ev-inventory:matBagUpdate', meta.id, meta); */
        return;
    }
...
tradingIn = false;
on('ev-cia:depositMaterials', async () => {
    if (tradingIn) return;
    tradingIn = true;
    emit('doAnim', 'cokecut');
    const finished = await new Promise((r) => exports['ev-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
    ClearPedTasks(PlayerPedId());
    tradingIn = false;
    if (finished !== 100) return;
    const qty = Math.min(exports['ev-inventory'].getQuantity('cia_weapon_material', true), 50000);
    if (qty < 1) return;
    emit('inventory:removeItem', 'cia_weapon_material', qty);
    TriggerServerEvent('ev-inventory:depositCiaMaterials', qty);
});

on('ev-cia:checkMaterials', async () => {
    if (tradingIn) return;
    TriggerServerEvent('ev-inventory:checkCiaMaterials');
});

on('ev-cia:getGear', async () => {
    let response = 1;
    while (response) {
        await new Promise(r => setTimeout(r, 250));
        response = await exports['ev-ui'].OpenContextMenu([
            {
                key: { item: '-820634585' },
                title: 'Taser',
            },
        ...
        if (!response) break;
        TriggerServerEvent('ev-inventory:ciaGetGear', response.item);
    }
});
