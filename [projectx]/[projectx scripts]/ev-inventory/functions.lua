...
function getQuantity(itemid, checkQuality, metaInformation)
    local amount = 0
    for i,v in pairs(clientInventory) do
        local qCheck = not checkQuality or (v.quality and v.quality > 0)
        if v.item_id == itemid and qCheck then
            if metaInformation then
                local totalMetaKeys = 0
                local metaFoundCount = 0
                -- SAFETY CHECK: skip if v.information is nil or not a string
                if type(v.information) ~= "string" or v.information == "" then goto continue end
                local ok, itemMeta = pcall(json.decode, v.information)
                if not ok or type(itemMeta) ~= "table" then goto continue end
                ...
                if totalMetaKeys <= metaFoundCount and v.amount then
