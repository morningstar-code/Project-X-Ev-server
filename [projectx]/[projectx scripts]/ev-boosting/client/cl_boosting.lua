...
local pVeh = nil
local isInBoost = false

function isInBoost() 
  return isInBoost
end
exports("isInBoost", isInBoost)
...
RegisterNetEvent('ev-boosting:RemoveBlip')
AddEventHandler('ev-boosting:RemoveBlip',function(playerID)
...
RegisterNetEvent('ev-boosting:updateBlips')
AddEventHandler('ev-boosting:updateBlips',function(info)
...
function FinishPolice() 
  if isVin then
    ...
    isPoliceBlip = false
    RemoveCopBlip()
    TriggerServerEvent("ev-boosting:SendRemovePoliceBlip", VehicleStoled)
  else
    isPoliceBlip = false
    RemoveCopBlip()
    TriggerServerEvent("ev-boosting:SendRemovePoliceBlip", VehicleStoled)
...
function DoHackingStages()
  if hacks.number1 == true then
    if CustomMinigameHack() then
      ...
      TriggerEvent('ev-boosting:addcooldownHack')
      return true
    else
...
function RemoveCopBlip()
  TriggerServerEvent("ev-boosting:SendRemovePoliceBlip")
end
    ...
    if IsInQueue then
      ...
      if IsInQueue then
        if not AlredyContract then
          TriggerServerEvent("ev-boosting:CreateContract", getClock())
        end
      end
    end
    Citizen.Wait(1000)
        ...
        if(GetVehicleNumberPlateText(veh) == placa) then
          ...
          if aDist < 10.0 and HackingIsCompleted() then
            if not notEmailed then
              notEmailed = true
              TriggerEvent('phone:emailReceived', 'boosting', 'Anonymous', 'Leave the vehicle and get out of the area.')
            end
            sendAppEvent('SetDisableButtons', {enableButtons = false})
            sendAppEvent("RemoveContractInfo", {IdToRemove = IdToRemove})
            TriggerServerEvent("ev-boosting:completeNormalContract", IdToRemove)
            IdToRemove = nil
            AlredyContract = false
            TriggerServerEvent("ev-boosting:IsInContract", false)
            Citizen.CreateThread(function()
                while notEmailed do
                  Wait(150)
                  if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), setCoords) >= 100.0 then
                    DeleteEntity(veh)
                    RemoveBlip(dropZoneblip)
                    TriggerEvent('phone:emailReceived', 'boosting', 'Anonymous', 'Boost Complete, thankyou hopefully I will see you again.')
...
function DropItemZone(data, pay)
  ...
  Citizen.CreateThread(function()
    while true do
      ...
      local aDist = GetDistanceBetweenCoords(setCoords, coordA)
      if aDist < 5.0 then
        TriggerServerEvent("ev-boosting:bennysitem", data, pay)
        RemoveBlip(itemBlip)
...
AddEventHandler("ev-boosting:characterSpawned", function(cid)
  local gne = RPC.execute("phone:getCryptoBalance", cid, 1)
...
RegisterNetEvent("ev-boosting:CreateContractFromServer")
AddEventHandler("ev-boosting:CreateContractFromServer", function(contract, extra)
  sendAppEvent('ContractsBoosting', {contracts = contract})
  sendAppEvent('BoostingNotification', {notify = extra})
end)

RegisterNetEvent("ev-boosting:CreateContractFromServer2")
AddEventHandler("ev-boosting:CreateContractFromServer2", function(contract, extra)
  sendAppEvent('ContractsBoosting', {contracts = contract})
end)

RegisterNetEvent("ev-boosting:updateTime")
AddEventHandler("ev-boosting:updateTime", function(id, time)
...
RegisterNetEvent("ev-boosting:setlevel")
AddEventHandler("ev-boosting:setlevel", function(pLevel, newgne)
  local level, type1, type2 = filterLevel(pLevel)
  gne = tonumber(newgne)
        ...
        if CustomMinigame() == true then
            ...
            if(GetVehicleNumberPlateText(vehicleHandle) == placa)then
              if isVin then
                  ...
                  TriggerEvent('ev-phone:SendNotify', 'Disable the tracker then head to the scratch location.', 'boosting', 'Anonymous')
                  MakeDropZoneThreadVin(true)
                  DeleteBlip()
                  TriggerServerEvent("Boosting:AddBlipsSystem")
                  TriggerServerEvent('ev-boosting:SetPlateInList', TrimValue(GetVehicleNumberPlateText(vehicleHandle)), true)
                  isPoliceBlip = true
              else
                  isInBoost = false
                  exports['ev-dispatch']:dispatchadd('10-99', 'Tracker Device Tampering - Grand Theft Auto', '227')
                  ...
                  TriggerEvent('phone:emailReceived', 'boosting', 'Anonymous', 'Disable the tracker then head to the drop off location.')
                  isPoliceBlip = true
                  DeleteBlip()
                  TriggerServerEvent("Boosting:AddBlipsSystem")
                  TriggerServerEvent('ev-boosting:SetPlateInList', TrimValue(GetVehicleNumberPlateText(vehicleHandle)), true)
...
RegisterNetEvent("ev-boosting:addcooldownHack")
AddEventHandler("ev-boosting:addcooldownHack", function()
  exports['ev-dispatch']:dispatchadd('10-99', 'Tracker Device Tampering - Grand Theft Auto', '227')
  cooldown = true
...
RegisterNetEvent("ev-boosting:helpHack")
AddEventHandler("ev-boosting:helpHack", function()
  DoHackingStagesCOP()
  if HackingIsCompleted() then
    if isVin then
      sendAppEvent('BoostingNotification', {notify = {msg = "Go to the marked place and scratch the vin", time = "3/4" , app = 'Boosting', img = 'https://imgur.com/FEPqpLc.png'}})
      TriggerEvent('DoLongHudText', 'Go to the marked place and scratch the vin', 1)
      isPoliceBlip = false
      TriggerServerEvent('ev-boosting:SetPlateInList', TrimValue(GetVehicleNumberPlateText(VehicleStoled)), false)
...
RegisterNetEvent("ev-boosting:antiPolice")
AddEventHandler("ev-boosting:antiPolice", function()
  local veh = GetVehiclePedIsIn(PlayerPedId())
  if IsPedInAnyVehicle(PlayerPedId()) then
    local result = RPC.execute("ev-boosting:GetPlateState", TrimValue(GetVehicleNumberPlateText(veh)))
    if result then
        ...
        if isPoliceBlip then
          if cooldown then
            ...
            if HackingIsCompleted() then
              if isVin then
                ...
                isPoliceBlip = false
                TriggerServerEvent("Boosting:RemoveBlipsSystem")
                TriggerServerEvent('ev-boosting:SetPlateInList', TrimValue(GetVehicleNumberPlateText(veh)), false)
              else
                isPoliceBlip = false
                TriggerServerEvent('ev-boosting:SetPlateInList', TrimValue(GetVehicleNumberPlateText(veh)), false)
                TriggerServerEvent("Boosting:RemoveBlipsSystem")
          ...
          if cooldown then
            ...
            if CustomMinigameHack() then
              ...
              TriggerServerEvent('ev-boosting:coop', GetPlayerServerId(NetworkGetPlayerIndexFromPed(GetPedInVehicleSeat(veh, -1))))
            else
              TriggerEvent('ev-boosting:addcooldownHack')
...
RegisterNetEvent("ev-boosting:copBoosting")
AddEventHandler("ev-boosting:copBoosting", function()
  DoHackingStagesCOP()
end)
...
AddEventHandler('ev-boosting:open:boostingApp', function()
  ...
  TriggerServerEvent("ev-boosting:getLevel")
  TriggerEvent("ev-boosting:characterSpawned", cid)
...
RegisterNetEvent("ev-boosting:sendHowMuchGNEIHave", function(pGne)
    gne = pGne
end)
    ...
    if data.IsVin then
      ...
      if units + tonumber(data.props.ExtraVin) <= tonumber(gne) then
        if exports['ev-inventory']:hasEnoughOfItem('trackerdisabler', 1) then
            ...
            AlredyContract = true
            TriggerServerEvent("ev-boosting:IsInContract", true)
            TriggerServerEvent("ev-boosting:RemoveBNE", units + tonumber(data.props.ExtraVin))
      ...
      if units <= tonumber(gne) then
        if exports['ev-inventory']:hasEnoughOfItem('trackerdisabler', 1) then
            TriggerServerEvent("ev-boosting:RemoveBNE", units)
            HackingStage(NumberOfContractsByLevel(data.props.type))
            AlredyContract = true
            TriggerServerEvent("ev-boosting:IsInContract", true)
            local vehicle, plate = CreateVeh(data.props.vehicle, data.props.coords)
            ...
            VehicleStoled = vehicle
            isInBoost = true
            sendAppEvent('BoostingNotification', {notify = {msg = 'Your contract has been started Information Plate: '.. tostring(plate), time = getClock() , app = 'Boosting', img = 'https://imgur.com/FEPqpLc.png'}})
            IdToRemove = data.props.id
            CreateBlip(data.props.coords)
            sendAppEvent('SetDisableButtons', {enableButtons = true})
            cb({Contract = true})
...
RegisterNUICallback("ev-boosting:transferContract", function(data, cb) 
  ...
  TriggerServerEvent("ev-boosting:TransferContract", data)
  cb({})
end)
...
RegisterNUICallback("ev-boosting:updateTime", function(data, cb) 
  ...
  TriggerServerEvent("ev-boosting:updateTimer", id, counter)
  cb({})
end)

RegisterNUICallback("ev-boosting:DeclineContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  TriggerServerEvent("ev-boosting:CancelContract", data)
  hacks = {
    number1 = false,
    number2 = false,
    number3 = false,
...
RegisterNUICallback("ev-boosting:JoinQueue", function(data, cb) 
  ...
  if IsInQueue then
    ...
    TriggerEvent("jobs:checkIn", "boosting")
  else
    ...
    TriggerEvent("ev-boosting:checkOut")
  end
  cb({})
end)
...
RegisterNUICallback("ev-purchaseGNE", function(data, cb) 
  local amount = tonumber(data.imputValue3)
  TriggerServerEvent("ev-boosting:purchaseGNE", amount)
  cb({})
end)

RegisterNUICallback("ev:sendgne", function(data, cb)
  local player = data.player
  local GneToSend = tonumber(data.GneToSend)
  -- TriggerServerEvent("ev-boosting:sendGNE", player, GneToSend)
  cb({})
end)

RegisterNetEvent("ev-boosting:FinishGNE")
AddEventHandler("ev-boosting:FinishGNE", function(pass, gne)
  if pass then
...
RegisterNetEvent("ev-boosting:updateGNE")
AddEventHandler("ev-boosting:updateGNE", function(gne)
  sendAppEvent('UpdateGnes', {gne = gne})
end)

RegisterNetEvent("ev-boosting:Notification")
AddEventHandler("ev-boosting:Notification", function(data)
  sendAppEvent('BoostingNotification', {notify = data})
end)

-- Laptop Interact --
RegisterNetEvent("ev-boosting:client:UseComputer")
AddEventHandler("ev-boosting:client:UseComputer" , function()
...
RegisterNetEvent('ev-boosting:finish_scratch')
AddEventHandler('ev-boosting:finish_scratch', function()
  local veh = pVeh
  local playerped = PlayerPedId()
  ...
  TriggerEvent('animation:PlayAnimation', 'kneel')
    ...
    if (finished == 100) then
      ...
      TriggerServerEvent("ev-boosting:completeVinContract", IdToRemove, placa)
      IdToRemove = nil
      AlredyContract = false
      TriggerServerEvent("ev-boosting:IsInContract", false)
