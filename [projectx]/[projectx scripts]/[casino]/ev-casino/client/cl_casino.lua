...
function drawCarForWins()
  ...
  if not carOnShow then
    -- carOnShow = RPC.execute('ev-casino:getRandomCar')
...
--   -- TriggerServerEvent("ev-bugs:playerLogAction", {
...
function enterCasino(pIsInCasino)
  ...
  if not inCasino then
    ...
    RPC.execute("ev-casino:getSpawnedPedCoords", false)
    TriggerEvent("ev-casino:casinoExitedEvent")
    TriggerServerEvent('ev-infinity:scopes:casino', false)
  ...
  TriggerServerEvent('ev-infinity:scopes:casino', true)
end
...
Citizen.CreateThread(function()
  ...
  local data = RPC.execute("ev-heists:getVaultLowerState")
...
function doSomeTextureReplacesBruv(apply)
  if not apply then
    ...
    if wheelDui then exports["ev-lib"]:releaseDui(wheelDui.id) end
    wheelDui = nil
    return
  end
...
function handlePedCoordsBaby(pPedCoords)
  ...
  RPC.execute("ev-casino:handoffPedData", myPeds)
  Citizen.CreateThread(function()
    while inCasino do
...
-- chips
RegisterUICallback("ev-ui:casinoPurchaseChipsAmount", function(data, cb)
...
RegisterUICallback("ev-ui:transferChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local stateId = data.values.stateId
  local amount = data.values.amount
...
AddEventHandler("ev-casino:purchaseChipsAction", function(pArgs)
  ...
  --   TriggerEvent("DoLongHudText", "Members only")
  --   return
  -- end
...
RegisterNetEvent("ev-casino:purchaseMembership", function() 
...
RegisterUICallback("ev-ui:casinoGetMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['ev-ui']:closeApplication('textbox')
...
AddEventHandler("ev-casino:purchaseMembershipCard", function()
  local characterId = exports["isPed"]:isPed("cid")
  RPC.execute("ev-casino:purchaseMembershipCard", characterId)
end)

AddEventHandler("ev-casino:getLoyaltyCard", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", characterId, "casino")
  if not jobAccess then
    TriggerEvent("DoLHudText", "Please talk to a member of floor staff", 2)
    return
  end
...
RegisterUICallback("ev-ui:casinoGetVipMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['ev-ui']:closeApplication('textbox')
...
function hasMembership(hrOnly)
  if exports['ev-config']:GetMiscConfig("casino.membership.bypass") then
    return true
  end
...
AddEventHandler("ev-casino:inVRHeadset", function(pInVRHeadset)
  inVRHeadset = pInVRHeadset
end)

RegisterUICallback("ev-ui:casinoCheckSpentWheelAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local stateId = data.values.state_id
  RPC.execute("ev-casino:checkSpentAmount", stateId)
  exports['ev-ui']:closeApplication('textbox')
end)
AddEventHandler("ev-casino:wheel:checkSpentAmount", function()
  ...
  exports['ev-ui']:openApplication('textbox', {
    callbackUrl = 'ev-ui:casinoCheckSpentWheelAmount',
    inputKey = 1,
    items = {
      {
