...
RegisterNetEvent("iconic_luckywheel:doRoll")
AddEventHandler("iconic_luckywheel:doRoll", function(_priceIndex) 
    _isRolling = true
	SetEntityHeading(_wheel, 0.9754)
    Citizen.CreateThread(function()
        local speedIntCnt = 1
...
RegisterNetEvent("iconic_luckywheel:rollFinished")
AddEventHandler("iconic_luckywheel:rollFinished", function() 
    _isRolling = false
end)
...
-- end)
-- local enabled = true
RegisterNetEvent("iconic_luckywheel:spinit")
AddEventHandler("iconic_luckywheel:spinit", function() 
    if not _isRolling  then
        doRoll()
...
--     local myJob = exports["iconic_handler"]:isPed("myJob")
...
RegisterNetEvent("attempt:change:spin", function()
	if exports['iconic_handler']:isPed("myjob") == "casino_dealer" then
