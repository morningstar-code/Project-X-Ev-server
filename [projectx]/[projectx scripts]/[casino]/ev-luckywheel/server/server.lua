...
RegisterServerEvent('iconic_luckywheel:getLucky')
AddEventHandler('iconic_luckywheel:getLucky', function(money)
    local src = source
        ...
        if user ~= nil then
            if user:getCash() >= amount  then
                ...
                SetTimeout(7000, function()
                    ...
                    TriggerClientEvent("iconic_luckywheel:rollFinished", -1)
                end)
                TriggerClientEvent("iconic_luckywheel:doRoll", -1, _priceIndex)
            else
                TriggerClientEvent("iconic_luckywheel:rollFinished", -1)  
...
RegisterServerEvent("attempt:spin", function()
    local pSrc = source
    if CasinoSpin then
        TriggerClientEvent("iconic_luckywheel:spinit", pSrc, true)
    else
