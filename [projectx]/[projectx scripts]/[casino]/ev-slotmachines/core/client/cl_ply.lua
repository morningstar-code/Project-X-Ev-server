...
local function drawStatusHUD(state, pValues)
  ...
  table.insert(values, "Jackpot Accumulator: ~$" .. currentAccJackpot)
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "ev-ui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "ev-ui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })
...
AddEventHandler("ev-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  ...
  while isSeatedAtSlotMachine do 
    ...
    if IsControlJustReleased(1, 202) then --Key: escape
...
AddEventHandler("ev-slotmachines-cl:stopReels", function(reel, rot)
  ...
  -- TriggerServerEvent("ev-slotmachines-sv:slotsCheckWin", rot)
end)
...
AddEventHandler("ev-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    ...
    TriggerEvent("DoLongHudText", "You've won $" .. amountWon .. "!")
  else
...
--#[NUI Callbacks]#--
RegisterUICallback("ev-ui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    print("ev-slotmachines | currentSlotMachineModel", currentSlotMachineModel)
    TriggerServerEvent(
...
RegisterUICallback("ev-ui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
end)
...
if GetConvar("sv_environment", "prod") == "debug" then
  RegisterCommand('inside', function(source, args, RawCommand)
    isInsideCasino = true
    isSlotMachinesEnabled, currentAccJackpot = RPC.execute("ev-slotmachines-sv:checkIfSlotMachinesState")
  ...
  RegisterCommand('reel', function(source, args, RawCommand)
    print(json.encode(currentReels))
    RPC.execute("setReels", currentReels)
  end)
end