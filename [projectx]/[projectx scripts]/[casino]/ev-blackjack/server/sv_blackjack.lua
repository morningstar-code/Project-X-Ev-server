...
RPC.register("ev-blackjack:getDealerPedSeeds", function(src)
    math.random() math.random() math.random()
    local DealersPedSeeds = {}
    local randomBlackShit = math.random(1,13)
    ...
    for i = 1, 7 do
        DealersPedSeeds = {
            [i + 1] = {
                seed = randomBlackShit,
                model = dealerModel,
            },
        }
    end
    Citizen.Wait(100)
    return DealersPedSeeds
end)

RegisterNetEvent("Blackjack:requestBlackjackTableData")
AddEventHandler("Blackjack:requestBlackjackTableData", function()
    local src = source
    TriggerClientEvent("Blackjack:sendBlackjackTableData", src, blackjackTables)
end)
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if src ~= nil then
        ...
        TriggerClientEvent("DoLongHudText", src, "can't sit you down.", 2)
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    ...
                    if blackjackGameData[gameId][src] == nil then
                        blackjackGameData[gameId][src] = {}
                    end
                    ...
                    TriggerClientEvent("DoLongHudText", src, "Bet placed: " .. tostring(betAmount) .. " chips.", 1)
                end
            end
        end
    else
        TriggerClientEvent("DoLongHudText", src, "Error betting!", 2)
    end
end)

RegisterNetEvent("Blackjack:hitBlackjack")
AddEventHandler("Blackjack:hitBlackjack", function(gameId, nextCardCount, UseChipsResult)
    local src = source
    blackjackGameData[gameId][src][2][nextCardCount] = true
end)

RegisterNetEvent("Blackjack:standBlackjack")
AddEventHandler("Blackjack:standBlackjack", function(gameId, nextCardCount, UseChipsResult)
    local src = source
    blackjackGameData[gameId][src][2][nextCardCount] = gameId
    -- TriggerClientEvent("Blackjack:standOrHitPrompt")
end)
                            ...
                            if blackjackTables[chairID] ~= false then
                                ...
                                if blackjackGameData[gameId][source] ~= nil then 
                                    ...
                                    if currentHand < 21 then
                                        ...
                                        TriggerClientEvent("Blackjack:standOrHitPrompt", source)                    
                                        ...
                                        if currentHand ~= nil then
                                            if currentHand <= 21 then
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    exports["ev-casino"]:giveChips(source, potentialWinAmount)
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            TriggerClientEvent("Blackjack:blackjackWin", source, tableId)
                                                            TriggerClientEvent("DoLongHudText", source, tostring(potentialWinAmount).." chips", 2)
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    exports["ev-casino"]:giveChips(source, potentialPushAmount)
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            TriggerClientEvent("Blackjack:blackjackPush", source, tableId)
                                                            TriggerClientEvent("DoLongHudText", source,"+0 chips", 2)
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("DoLongHudText", source, tostring(potentialPushAmount).." chips", 2)
                                                            -- if potentialPushAmount > 10000000 then
...
RegisterCommand('blackjack', function(source, args, RawCommand)
    print(json.encode(blackjackGameData))
end)
