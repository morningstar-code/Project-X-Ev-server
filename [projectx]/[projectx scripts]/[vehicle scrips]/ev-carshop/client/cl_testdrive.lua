...
RegisterNetEvent('ev-showrooms:getCars', function()
  -- local cars = RPC.execute('ev-showrooms:fetchCars')
  ...
  for idx, car in ipairs(cars) do
    ...
    if IsModelInCdimage(model) and IsModelAVehicle(model) then
      ...
      Wait(100)
      local class = exports['ev-vehicles']:GetVehicleRatingClass(tempCar)
      if not class then
  ...
  TriggerServerEvent('ev-showrooms:saveCars', new)
end)
