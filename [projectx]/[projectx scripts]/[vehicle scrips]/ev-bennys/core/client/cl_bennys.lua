...
local bennysLocations = {
    ...
    ["bennysmrpd"] = {
        pos = vector3(455.21, -974.69, 25.13),
        heading = 359.69,
        access = 'police'
    },
    ["bennysvbpd"] = {
...
local function saveVehicle()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        TriggerEvent("ev-jobs:bennys:order", veh, changedStuff)
        return
    end
...
function AttemptPurchase(purchaseType, upgradeLevel)
    ...
    if bennys.access and (bennys.access == "police" or bennys.access == "ems") and purchaseType == "repair" then
        return RPC.execute("ev-bennys:jobRepair")
    end
    ...
    if (bennys.access == "police" or bennys.access == "ems") then
        local free = true --exports['ev-config']:GetMiscConfig('bennys.pd.free')
        pd = free
        return pd
    end

    TriggerServerEvent("ev-bennys:attemptPurchase", cheap, purchaseType, upgradeLevel)
...
function RepairVehicle()
    Citizen.CreateThread(function ()
        ...
        if (missingEngineHealth > 50) then
            local finished = exports["ev-taskbar"]:taskBar(5000 + (missingEngineHealth / 50), "Repairing engine...", true)
        ...
        if missingBodyHealth > 50 then
            local finished = exports["ev-taskbar"]:taskBar(5000 + (missingBodyHealth / 50), "Repairing body...", true)
...
function PreviewMod(categoryID, modID)
    ...
    if categoryID == 39 or categoryID == 4 then
        if GetResourceState("ev-vehiclesync-state") == "started" then
            exports['ev-vehiclesync-state']:SetEngineSound(plyVeh, true)
        elseif GetResourceState("ev-vehiclesync") == "started" then
            exports['ev-vehiclesync']:SetEngineSound(plyVeh, true)
        end
    end
end
...
function ExitBennys()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        if GetResourceState("ev-vehiclesync-state") == "started" then
            exports['ev-vehiclesync-state']:SetEngineSound(plyVeh)
        elseif GetResourceState("ev-vehiclesync") == "started" then
            exports['ev-vehiclesync']:SetEngineSound(plyVeh)
        end
    ...
    elseif not isAtPreviewBennys then
      exports["ev-ui"]:showInteraction("Bennys")
...
function enterLocation(pBennys, needsRepair)
    ...
    if isAtEmployedBennys then
        ...
        VehicleClass = exports["ev-vehicles"]:GetVehicleRatingClass(plyVeh)
    end
    ...
    isVinScratched = exports['ev-vehicles']:IsVinScratched(plyVeh)
...
AddEventHandler("bennys:enter", function()
    ...
    if bennysAccess then
        ...
        jobAccess = exports["ev-business"]:IsEmployedAt(bennysAccess)

        if (isAtPreviewBennys and previewPermission[currentBennys] and isTestDrive) then
            jobAccess = exports["ev-business"]:HasPermission(bennysAccess, previewPermission[currentBennys])
        end
...
Citizen.CreateThread(function()
    Wait(5000)
    local catalog = RPC.execute('ev-jobs:bennys:getPartsCatalog')

    local modifiers = RPC.execute('ev-jobs:bennys:getPriceModifiers')

    local taxLevel = RPC.execute("ev-financials:getTax", "Services")
...
RegisterNetEvent('ev-admin:bennys', function()
    devmode = true
