...
RegisterServerEvent('ev-bennys:removeFromInUse')
AddEventHandler('ev-bennys:removeFromInUse', function(currentBennys)
    Bennys[currentBennys] = nil
end)

RegisterServerEvent('ev-bennys:addToInUse')
AddEventHandler('ev-bennys:addToInUse', function(currentBennys)
...
AddEventHandler('ev-bennys:repairciv', function()
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(450)
end)

RPC.register("ev-jobs:bennys:getPartsCatalog", function(src)
    return PartsCatalog
end)

RPC.register("ev-jobs:bennys:getPriceModifiers", function(src)
    return PriceModifiers
end)

RPC.register("ev-bennys:jobRepair", function(src)
    return true
end)

RPC.register("ev-bennys:checkIfUsed", function(src, currentBennys)
