...
function SetEngineSound(pVehicle, pIsTemporary)
    ...
    if not sound then
        sound = exports['ev-vehicles']:GetVehicleAfterMarket(netId, 'engineSound')
    end
...
function ApplyEngineSound(pVehicle, pSound)
    ForceVehicleEngineAudio(pVehicle, pSound)
end

exports('SetEngineSound', SetEngineSound)
exports('ApplyEngineSound', ApplyEngineSound)
...
function UpdateSyncState(pVehicle, pSyncState)
    ...
    if flags['wheelFitment'] ~= previous['wheelFitment'] then
        local wheelFitment = exports['ev-vehicles']:GetVehicleWheelFitment(pVehicle)
        if wheelFitment and wheelFitment.width then
            if not DecorExistOn(pVehicle, "ev-wheelfitment_applied") then
                DecorSetBool(pVehicle, "ev-wheelfitment_applied", true)
            end

            DecorSetFloat(pVehicle, "ev-wheelfitment_w_width", wheelFitment.width)
            DecorSetFloat(pVehicle, "ev-wheelfitment_w_fl", wheelFitment.fl)
            DecorSetFloat(pVehicle, "ev-wheelfitment_w_fr", wheelFitment.fr)
            DecorSetFloat(pVehicle, "ev-wheelfitment_w_rl", wheelFitment.rl)
            DecorSetFloat(pVehicle, "ev-wheelfitment_w_rr", wheelFitment.rr)
        end
    end
...
RegisterNetEvent('ev-vehicles:spawnedVehicle', function(pNetId)
    Citizen.Wait(1000)
    ...
    local afterMarkets = exports['ev-vehicles']:GetVehicleAfterMarket(pNetId)
    ...
    local wheelFitment = exports['ev-vehicles']:GetVehicleWheelFitment(pNetId)
...
RegisterNetEvent('ev-vehicles:updateAfterMarkets', function(pNetId)
    Citizen.Wait(1000)
    local vehicle = NetworkGetEntityFromNetworkId(pNetId)
