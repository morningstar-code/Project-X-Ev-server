...
local function showbicycleMenu()
  ...
  for _, vehicle in pairs(bicyclesList) do
    data[#data + 1] = {
      ...
      key = vehicle.model,
      children = {
          { title = ("Confirm Purchase"), icon = "check-circle", action = "ev-ui:bicyclePurchase", key = vehicle.model },
          { title = ("Cancel Purchase"), icon = "times-circle", action = "ev-ui:bicyclecancelPurchase", key = nil },
      },
    }
  end
...
RegisterUICallback("ev-ui:bicyclePurchase", function(data, cb)
  local bicycleShop = getClosestbicycleShop()
  if bicycleShop == nil then return end
  ...
  local success, message = RPC.execute("bicycles:purchaseVehicle", d)
  if not success then
  ...
  TriggerServerEvent("ev:vehicles:bicyclespawn", model, coords, bicycleShop.heading)
  local rentalInfo = RPC.execute("ev:vehicles:bicyclespawn", model, coords, bicycleShop.heading)

  cb({ data = {}, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("ev-ui:bicyclecancelPurchase", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
end)
