...
RegisterNetEvent('ev-rcvehicles:car:setup', function(pCarSettings)
...
RegisterNetEvent('ev-rcvehicles:car:spawn', function(pCarSettings)
...
RegisterNetEvent('ev-rcvehicles:car:pilot', function(pCarId)
  ...
  if not DoesEntityExist(CURRENT_CAR.handle) then
    DoScreenFadeIn(400)
    RPC.execute('ev-rcvehicles:leaveCar', CURRENT_CAR.id)
  ...
  TriggerEvent('ev-rc:carEntered', CURRENT_CAR)
  ...
  TriggerEvent('ev-ui:setVehicleBypassed', CURRENT_CAR.handle, true)
  DoScreenFadeIn(1000)
  ...
  Citizen.CreateThread(function()
    ...
    TriggerEvent('ev-ui:setVehicleBypassed', CURRENT_CAR.handle)

    SetNetworkIdCanMigrate(CURRENT_CAR.netId, true)

    TriggerEvent('ev-rc:carExited', CURRENT_CAR)
    RPC.execute('ev-rcvehicles:leaveCar', CURRENT_CAR.id)
    if CURRENT_CAR.crashed then
      RPC.execute('ev-rcvehicles:crashCar', CURRENT_CAR.id)
    end
    CURRENT_CAR = nil
  end)
...
RegisterNetEvent('ev-rcvehicles:car:destroy', function(pCarId)
