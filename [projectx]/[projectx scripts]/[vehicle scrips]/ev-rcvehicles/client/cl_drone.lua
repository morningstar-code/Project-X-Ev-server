...
RegisterNetEvent('ev-rcvehicles:drone:spawn', function(pDroneSettings)
...
RegisterNetEvent('ev-rcvehicles:drone:pilot', function(pDroneId)
  ...
  TriggerEvent('ev-rc:droneEntered', CURRENT_DRONE)
  ...
  Citizen.CreateThread(function()
    ...
    while isPiloting do
      ...
      if IsDisabledControlJustPressed(0, Keys['SPACE']) then
        TriggerEvent('ev-rc:droneAbility', CURRENT_DRONE)
      end
    ...
    TriggerEvent('ev-rc:droneExited', CURRENT_DRONE)
    RPC.execute('ev-rcvehicles:leaveDrone', CURRENT_DRONE.id)
    if CURRENT_DRONE.crashed then
        RPC.execute('ev-rcvehicles:crashDrone', CURRENT_DRONE.id)
    end
    CURRENT_DRONE = nil
  end)
...
RegisterNetEvent('ev-rcvehicles:drone:destroy', function(pDroneId)
