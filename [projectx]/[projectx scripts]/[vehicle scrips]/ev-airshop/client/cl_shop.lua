local airList = {
  ...
  local function showairMenu()
    ...
    for _, vehicle in pairs(airList) do
      data[#data + 1] = {
        ...
        key = vehicle.model,
        children = {
            { title = _L("air-confirm-purchase", "Confirm Purchase"), icon = "check-circle", action = "ev-ui:airPurchase", key = vehicle.model },
            { title = _L("air-confirm-purchase", "Cancel Purchase"), icon = "times-circle", action = "ev-ui:aircancelPurchase", key = nil },
        },
      }
    end
  ...
  RegisterUICallback("ev-ui:airPurchase", function(data, cb)
    local airShop = getClosestairShop()
    if airShop == nil then return end
    cb({ data = {}, meta = { ok = true, message = "done" } })
    local finished = exports["ev-taskbar"]:taskBar(15000, "Purchasing...", false, true, false, false, nil, 2.5, PlayerPedId())
    ...
    local success, message = RPC.execute("air:purchaseVehicle", d)
    if not success then
    ...
    TriggerServerEvent("ev:vehicles:airpawn", model, coords, airShop.heading)
    local rentalInfo = RPC.execute("ev:vehicles:airpawn", model, coords, airShop.heading)
  
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterUICallback("ev-ui:aircancelPurchase", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)
