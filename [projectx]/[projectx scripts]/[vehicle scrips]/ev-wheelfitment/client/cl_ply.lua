...
local validZones = {
    ["ev-wheelfitment:zone1"] = true,
    ["ev-wheelfitment:zone2"] = true
}
...
function SyncWheelFitment()
    ...
    if didPlyAdjustFitments then
        ...
        local vin = exports["ev-vehicles"]:GetVehicleIdentifier(plyVeh)
        RPC.execute("ev-wheelfitment_sv:saveWheelfitment", vin, currentFitmentsToSet)
...
Citizen.CreateThread(function()
    exports["ev-polyzone"]:AddBoxZone("ev-wheelfitment:zone1", vector3(124.5, -3047.7, 7.05), 6.4, 4.0, {heading = 270.0, minZ = 6.04, maxZ = 9.04, data = {id = "wheel_fitment_zone"}})
    
    -- PD Zone bruh
    exports["ev-polyzone"]:AddBoxZone("ev-wheelfitment:zone2", vector3(435.41, -975.99, 25.7), 4.0, 8.6, {
    name="2",
    heading=0,
    minZ=24.5,
    maxZ=27.1,
...
AddEventHandler("ev-polyzone:enter", function(zone, data)
    ...
    Citizen.CreateThread(function()
        ...
        local isWheelFitmentInUse = RPC.execute("ev-wheelfitment_sv:getIsWheelFitmentInUse")
        local job = exports["ev-business"]:IsEmployedAt("tuner")
        while inZone and job do 
...
AddEventHandler("ev-wheelfitment_cl:applySavedWheelFitment", function(wheelFitments, pNetId)
    if wheelFitments ~= nil then return end
    
    performVehicleCheck = false
    local plyVeh = NetworkGetEntityFromNetworkId(pNetId)
