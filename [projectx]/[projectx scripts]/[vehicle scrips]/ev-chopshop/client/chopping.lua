...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["ev-taskbar"]:taskBar(10000,"Scrapping Car")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    local finished = exports["ev-taskbar"]:taskBar(14000,"Scrapping Car")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    local finished = exports["ev-taskbar"]:taskBar(25000,"Scrapping Car")
            ...
            if not IsInsideVehicle and distance and distance <= 10.0 then
                ...
                if inDistance then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if success then
                            RPC.execute("ev-jobs:chopshop:chopVehicle", vehicleModel, boneType) -- listId

                            if not exports["ev-flags"]:HasVehicleFlag(vehicle, "isScrapVehicle") then
                                exports["ev-flags"]:SetVehicleFlag(vehicle, "isScrapVehicle", true)
                            end
                        end
                    end
                end
