...
local function showBoatMenu()
  ...
  for _, vehicle in pairs(boatList) do
    data[#data + 1] = {
      ...
      key = vehicle.model,
      children = {
          { title = ("Confirm Purchase"), icon = "check-circle", action = "ev-ui:boatPurchase", key = vehicle.model },
          { title = ("Cancel Purchase"), icon = "times-circle", action = "ev-ui:boatcancelPurchase", key = nil },
      },
    }
  end
...
RegisterUICallback("ev-ui:boatPurchase", function(data, cb)
  local boatshop = getClosestboatshop()
  if boatshop == nil then return end
  ...
  local success, message = RPC.execute("boats:purchaseVehicle", d)
  if not success then
  ...
  TriggerServerEvent("ev:vehicles:boatspawn", model, coords, boatshop.heading)
  local rentalInfo = RPC.execute("ev:vehicles:boatspawn", model, coords, boatshop.heading)
...
RegisterUICallback("ev-ui:boatcancelPurchase", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
end)
