...
RegisterNetEvent("ev-clothing:isPedFreemodeModel")
AddEventHandler("ev-clothing:isPedFreemodeModel", function()
...
function getPedAppearance(ped)
    return {
        ...
        headBlend = exports["ev-clothing"]:getPedHeadBlend(ped),
        faceFeatures = exports["ev-clothing"]:getPedFaceFeatures(ped),
        headOverlays = exports["ev-clothing"]:getPedHeadOverlays(ped),
        components = getPedComponents(ped),
        props = exports["ev-clothing"]:getPedProps(ped),
        hair = getPedHair(ped),
...
function setPedAppearance(player, data)
    ...
    Citizen.CreateThread(function()    
        ...
        if data.faceFeatures then
            exports["ev-clothing"]:setPedFaceFeaturesJS(PlayerPedId(), data.faceFeatures)
        end

        if data.headBlend then
            exports["ev-clothing"]:setPedHeadBlend(PlayerPedId(), data.headBlend)
        end

        if data.props then
            exports["ev-clothing"]:setPedProps(PlayerPedId(), data.props)
        end

        if data.headOverlays then
            exports["ev-clothing"]:setPedHeadOverlays(PlayerPedId(), data.headOverlays)
        end
    ...
    TriggerServerEvent('ev-walkstyles:server:walkstyles', 'get')
    playerTattoos = {}
end
