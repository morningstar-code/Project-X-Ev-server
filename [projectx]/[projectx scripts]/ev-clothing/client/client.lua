...
CreateThread(function()
    local TextureNames = RPC.execute("ev-clothing:getTextureNames")
    Config.SetShops()
    Citizen.Wait(500)
    return TextureNames
end)
...
RegisterNetEvent("ev-clothing:applyCurrentClothing")
AddEventHandler("ev-clothing:applyCurrentClothing", function()
...
RegisterNUICallback("closeNui", function(data)
    ...
    SendReactMessage("royalFix")
    exports["ev-clothing"]:closeCamera()
    ...
    if data.buy and data.amount then
        local canPurchase = RPC.execute("ev-clothing:purchaseClothes", data.type, data.amount, isSpawn) 
        if canPurchase then
            TriggerEvent("DoLongHudText","Successfully Purchased.", 1)
            if inSpawn then
                TriggerEvent("ev-spawn:finishedClothing","Finished")
            end
           TriggerServerEvent('ev-clothing:saveSkin', getPedAppearance(PlayerPedId()))
...
function OpenShops(type, bool)
    ...
    exports["ev-clothing"]:setCamera(3)
...
RegisterNetEvent("ev-clothing:opeev-clothing", OpenShops)
