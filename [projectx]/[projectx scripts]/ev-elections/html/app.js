...
document.onkeydown = function(data){
    if (data.which == 27){
        ...
        $.post('https://ev-elections/close')
    }
}


function SetupData(){
    $.post('https://ev-elections/getConfig', JSON.stringify({}), function(vCode){
...
$(document).on('click','.Vote', function(){
    if (selected != ''){
        $.post('https://ev-elections/voteForSomeone', JSON.stringify({id:selected}))
        $('.bottompart').hide()
