...
RegisterNetEvent("ev-dispatch:911")
AddEventHandler("ev-dispatch:911", function(msg)
    local locationInfo = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local currentPos = GetEntityCoords(PlayerPedId())

    TriggerEvent("animation:PlayAnimation", "phone")
    local finished = exports["ev-taskbar"]:taskBar(math.random(2500, 4000), "Calling 911")
    ...
    if finished == 100 then
         TriggerServerEvent("dispatch:svNotify", {
             ...
             number =  exports["isPed"]:isPed("phone_number"),
             job = {"police"},
             information = msg
         })
      end
...
RegisterNetEvent("ev-dispatch:311")
AddEventHandler("ev-dispatch:311", function(source, args, rawCommand)
    local locationInfo = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local currentPos = GetEntityCoords(PlayerPedId())

    TriggerEvent("animation:PlayAnimation", "phone")
    local finished = exports["ev-taskbar"]:taskBar(math.random(2500, 4000), "Calling 311")
    TriggerEvent("animation:PlayAnimation", "cancel")
