...
function RefreshPeekList()
    ...
    Citizen.SetTimeout(250, function()
        ...
        exports["ev-ui"]:sendAppEvent('eye', {
            action = "refresh",
            payload = entries or {}
        })
    end)
...
function StartPeekin()
    ...
    Citizen.CreateThread(function()
        local rgbGreen, rgbWhite = {135, 170, 216}, {255, 255, 255}  
        local previousZones = {}
        local drawingZones = {}
        ...
        while IsPeeking do
            ...
            if IsPeakActive and (IsControlJustReleased(0, 24) or IsDisabledControlJustReleased(0, 24)) then
                ...
                exports["ev-ui"]:openApplication("eye", {
                    action = "interact",
    ...
    exports["ev-ui"]:sendAppEvent('eye', { action = "peek", payload = { display = true, active = false } })
end
...
RegisterNetEvent('ev-interact:ResetUI')
AddEventHandler('ev-interact:ResetUI', function()
    IsPeeking = false
...
RegisterNetEvent('ev-interact:detali')
AddEventHandler('ev-interact:detali', function()
    IsPeeking = false
    exports["ev-ui"]:sendAppEvent('eye', {
        action = "peek",
        payload = { display = false, active = false}
...
function CanSellOrBuyCar(pEntity, pSell)
  ...
  if pSell and vehicleOwner == 2 then --if you want to sell it it needs to return 2 (set sellable colum to 2)
    return true 
  end
  if not pSell and vehicleOwner ~= 2 and vehicleOwner ~= -1 then --Want to buy it then aslong as return isnt 2 or -1 you can.
    return true 
  end
...
AddEventHandler("ev:target:changed", function(pTarget, pEntityType, pEntityOffset)
...
AddEventHandler('ev-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
RegisterUICallback("ev-ui:targetSelectOption", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    IsPeeking = false

    exports["ev-ui"]:closeApplication("eye")
    ...
    local CPXEvent = option.CPXEvent
    local target = data.entity or 0
    local parameters = option.parameters or {}

    if event == nil and CPXEvent ~= nil then
        -- return CPX.Events.emit(CPXEvent, parameters, target, context)
        return TriggerEvent(CPXEvent, parameters, target, context)
    end

    TriggerEvent(event, parameters, target, context)
end)

AddEventHandler('ev-ui:phoneReady', function()
    RefreshPeekList()
end)

AddEventHandler("ev-ui:application-closed", function (name, data)
    if name ~= "eye" then return end
    IsPeeking = false
end)
...
Citizen.CreateThread(function()
    exports["ev-keybinds"]:registerKeyMapping("", "Player", "Peek at Target", "+targetInteract", "-targetInteract", "LMENU")
end)
