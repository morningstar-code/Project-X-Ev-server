...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    elseif context.type == 2 then
        context.meta = exports['ev-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['ev-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.meta = exports['ev-objects']:GetObjectByEntity(pEntity)
...
exports('isPersonBeingHeldUp', isPersonBeingHeldUp)
...
function hasJobPermission(pJob, pPermission)
    ...
    local hasJobPermission = function()
        if pPermission == "employee" then
            ...
            return RPC.execute('ev-business:hasPermission', pJob, pPermission, characterId)
        end
    end
...
function getVehicleOwner(pEntity)
    ...
    print("Owner Status")
    print(vehicleOwner) --Returns fuckk all stupid cunting dump RPC bollocks. NIL/NULL
...
AddEventHandler("ev-interact:openFridge", function()
    TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("ev-interact:grabDrink", function(args, entity, context)
    TriggerEvent("server-inventory-open", "1", context.zones["bar:grabDrink"].name)
end)

RegisterNetEvent('ev-interact:setNetVehicleBypassed', function(netId, bypass)
