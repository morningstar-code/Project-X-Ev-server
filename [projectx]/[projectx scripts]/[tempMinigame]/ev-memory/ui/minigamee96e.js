...
let mode = size;
let mode_data = {};
mode_data[5] = [Squares, '92px'];
mode_data[6] = [Squares, '74px'];
mode_data[7] = [Squares, '61px'];
mode_data[8] = [Squares, '51px'];
mode_data[9] = [Squares, '44px'];
mode_data[10] = [Squares, '38px'];
...
function check(){
    if(wrong === wrongNumber){
        ...
        $.post('http://ev-memory/callback', JSON.stringify({'success': false}));
        setTimeout(function () {
    ...
    if(right === mode_data[mode][0]){
        ...
        $('.splash').removeClass('hidden');
        $('.groups').addClass('hidden');
        $.post('http://ev-memory/callback', JSON.stringify({'success': true, cb:submiturl }));
        setTimeout(function () {
            $(".minigame").css("display", "none");
            location.reload()
        },1500)
