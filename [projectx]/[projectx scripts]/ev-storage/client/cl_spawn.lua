...
function placeContainer(selection, heading)
    ...
    if dist > 50 then
        return TriggerEvent("DoLongHudText", "You cannot place the container this far away", 2)
    end

    if getClosestStash(5, coordinates) ~= nil then
        return TriggerEvent("DoLongHudText", "You are too close to another container, give it some room", 2)
    end

    isPlacing = true

    if dist > 3 then
        TriggerEvent("DoLongHudText", "Go to the location to place it down!")
        CreateThread(function ()
...
function spawn(coordinates)
    local location = exports["ev-housing"]:getCurrentLocation()
    if location then
        local canPlace = canPlaceInHouse(location)
        if not canPlace then
            return TriggerEvent("DoLongHudText", "You cannot place the container here", 2)
        end
    ...
    TriggerServerEvent("ev-storage:prepareStorage", isSelecting, cid, coordinates, heading)
