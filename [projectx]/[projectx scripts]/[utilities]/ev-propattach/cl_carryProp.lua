...
-- Fuel_Nozel
attachedPropFuelNozel = 0
function removeAttachedPropFuelNozel()
	if DoesEntityExist(attachedPropFuelNozel) then
		DeleteEntity(attachedPropFuelNozel)
		attachedPropFuelNozel = 0
	end
end

RegisterNetEvent('destroyPropFuelNozel')
AddEventHandler('destroyPropFuelNozel', function()
	removeAttachedPropFuelNozel()
end)
...
RegisterNetEvent('attachPropFuelNozel')
AddEventHandler('attachPropFuelNozel', function(attachModelSent,x,y,z,xR,yR,zR)
	removeAttachedPropFuelNozel()
	attachModelFuelNozel = GetHashKey(attachModelSent)
	local bone = GetPedBoneIndex(PlayerPedId(), 57005)
	local coords = GetEntityCoords(PlayerPedId(), false)
	RequestModel(attachModelFuelNozel)
	while not HasModelLoaded(attachModelFuelNozel) do
		Citizen.Wait(100)
	end
	attachedPropFuelNozel = CreateObject(attachModelFuelNozel, coords.x, coords.y, coords.z, true, false, false)
	AttachEntityToEntity(attachedPropFuelNozel, PlayerPedId(), bone, 0.09, 0.05, -0.01, -93, -84, 1, true, true, false, true, 0, true)
end)
...
AddEventHandler('attachPropTest', function(item)
	TriggerEvent("attachItemPerm", item)
end)
...
RegisterNetEvent('attachItemFuelNozel')
AddEventHandler('attachItemFuelNozel', function(item)
	TriggerEvent("attachPropFuelNozel",attachPropList[item]["model"], attachPropList[item]["x"], attachPropList[item]["y"], attachPropList[item]["z"], attachPropList[item]["xR"], attachPropList[item]["yR"], attachPropList[item]["zR"])
end)
