RegisterNetEvent("ev-fx:smoke:grenade", function(pCoords)
    local players = exports["ev-infinity"]:GetNerbyPlayers(pCoords, 100)
...
RegisterNetEvent("fx:ThermiteChargeEnt", function(pNetId, pCoords)
    ...
    local players = exports["ev-infinity"]:GetNerbyPlayers(pCoords, 25)
...
RegisterNetEvent("fx:waterPour", function(pNetId)
    ...
    local players = exports["ev-infinity"]:GetNerbyPlayers(pCoords, 10)
...
RegisterNetEvent("ev-fx:graffiti:spray", function(pNetId, coords)
    local src = source
...
RegisterNetEvent("ev-fx:graffiti:stopSpray", function(pNetId)
    local pEntity = NetworkGetEntityFromNetworkId(pNetId)
    local pCoords = GetEntityCoords(pEntity)
...
RegisterNetEvent("ev-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrengh, pScale)
...
local function doFx(fx, i, offset)
    Citizen.CreateThread(function()
        ...
        TriggerEvent("ev-scenes:addSceneToClient", {
...
RegisterServerEvent("ev-fx:sound:playDispatch")
AddEventHandler("ev-fx:sound:playDispatch", function(sound)
    local src = source
    local coords = GetEntityCoords(GetPlayerPed(src))
    local players = exports["ev-infinity"]:GetNerbyPlayers(coords, 5)
    
    -- triggerAudio(src, 1, 2.0, sound, 'DLC_NIKEZ_GENERAL', 'ev-fx:sound:PlaySound')
    triggerAudio(src, 1, 2.0, sound, 'DLC_NIKEZ_RINGTONES', 'ev-fx:sound:PlaySound')
end)
...
RegisterServerEvent("ev-fx:sound:vehicles")
AddEventHandler("ev-fx:sound:vehicles", function(sound)
    local src = source
    local coords = GetEntityCoords(GetPlayerPed(src))
    local players = exports["ev-infinity"]:GetNerbyPlayers(coords, 3)

    triggerAudio(src, 1, 3.0, sound, 'DLC_NIKEZ_GENERAL', 'ev-fx:sound:PlaySound')
end)

RegisterServerEvent("ev-fx:sound:playGateLocking")
AddEventHandler("ev-fx:sound:playGateLocking", function(isLocked, isSpecial)
    local src = source
    local coords = GetEntityCoords(GetPlayerPed(src))
    local players = exports["ev-infinity"]:GetNerbyPlayers(coords, 3)
    local sound = ""
...
function triggerAudio(pPlayerId, pType, pRadius, pSound, pRef, pEvent)
    ...
    local Event = {
        ...
        outEvent = 'ev-fx:sound:StopSound'
    }

    return exports["ev-infinity"]:TriggerActiveAreaEvent(Event, Area)
end
