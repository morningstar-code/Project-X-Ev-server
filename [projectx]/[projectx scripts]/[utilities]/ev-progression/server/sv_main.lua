...
RPC.register("ev:progression:character:init", function(src)
    local user = exports["ev-base"]:getModule("Player"):GetUser(src) 
    local cid = user:getCurrentCharacter().id
    return GetProg(src, cid)
end)

RPC.register("ev:progression:character:illegalRep", function(src, amount, _type)
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
...
RPC.register("ev-progression:CheckRep", function (src, pRep)
...
function GetProg(src, cid)
    ...
    local progression = MySQL.query.await([[
        SELECT * FROM progression
        WHERE cid = ?
    ]], { cid })

    if not progression[1] then return GetNewPrgo(src, cid), print("ev-progression | error fetching progression data") end
