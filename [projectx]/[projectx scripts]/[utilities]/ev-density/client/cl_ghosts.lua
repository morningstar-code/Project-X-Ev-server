...
AddEventHandler('ev-config:configLoaded', function (pId)
                ...
                if inside and not Ghosted[vehicle] then
                    ...
                    if not isPlayer then
                        ...
                        if Config.antiGhostYeetVehicles then
                            TriggerEvent('ev-density:yeet', vehicle)
                        end
                    end
...
AddEventHandler('ev-density:yeet', function (vehicle)
    local vin = exports['ev-vehicles']:GetVehicleIdentifier(vehicle)

    if vin then return end

    exports['ev-sync']:SyncedExecution('DeleteEntity', vehicle)
end)
