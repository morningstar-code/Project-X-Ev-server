...
AddEventHandler('ev-gopro:activateVRChair', function ()
  density = 0.0
  print('pausing polulation', density)
end)
...
AddEventHandler('ev-density:disable', function (pDensity, pRogue)
...
Citizen.CreateThread(function()
    while true do
      ...
      if #toDelete > 0 then
        TriggerServerEvent('ev:peds:rogue', toDelete)
      end
...
RegisterNetEvent('ev:peds:rogue:delete')
AddEventHandler('ev:peds:rogue:delete', function(pNetId)
...
RegisterNetEvent('ev:peds:decor:set')
AddEventHandler('ev:peds:decor:set', function (pNetId, pType, pProperty, pValue)
  local entity = NetworkGetEntityFromNetworkId(pNetId)
