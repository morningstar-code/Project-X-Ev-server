...
RegisterCommand('clearAreaOf', function(src, args)
    ...
    if clearType == nil or clearType == 'everything' then
        TriggerClientEvent('ev-cleanup:clearAreaOfEverything', -1, vectors, radius + 0.0)
    elseif clearType == 'objects' then
        TriggerClientEvent('ev-cleanup:clearAreaOfObjects', -1, vectors, radius + 0.0)
    elseif clearType == 'vehicles' then
        TriggerClientEvent('ev-cleanup:clearAreaOfVehicles', -1, vectors, radius + 0.0)
    elseif clearType == 'peds' then
        TriggerClientEvent('ev-cleanup:clearAreaOfPeds', -1, vectors, radius + 0.0)
    end
end)
...
--[[ Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent('ev-cleanup:clearAreaOfEverything', -1, coords, 250000 + 0.0)
       Citizen.Wait(sleep) 
    end
 end) ]]