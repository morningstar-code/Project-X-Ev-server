...
AddEventHandler("ev-cleanup:enableCleanup", function(pEnabled)
  skipCleanupChecks = not pEnabled
end)

AddEventHandler("ev-cleanup:addBlacklistedProp", function(pProp)
...
local bypassObjects = {}
AddEventHandler('ev-objects:objectCreated', function(pObject, pHandle)
    if pHandle == nil then return end
    bypassObjects[pHandle] = true
end)

AddEventHandler('ev-objects:objectDeleted', function(pObject, pHandle)
    if pHandle == nil then return end
    bypassObjects[pHandle] = nil
end)

AddEventHandler('ev-objects:objectRecreating', function(pObject, pHandle)
    if pHandle == nil then return end
    bypassObjects[pHandle] = nil
end)

AddEventHandler('ev-objects:objectRecreated', function(pObject, pHandle)
    if pHandle == nil then return end
    bypassObjects[pHandle] = true
end)
...
RegisterNetEvent('ev-cleanup:clearAreaOfEverything')
AddEventHandler('ev-cleanup:clearAreaOfEverything', function (vectors, radius)
    ClearAreaOfEverything(vectors.x, vectors.y, vectors.z, radius, false, false, false, false)
end)

RegisterNetEvent('ev-cleanup:clearAreaOfObjects')
AddEventHandler('ev-cleanup:clearAreaOfObjects', function (vectors, radius)
    ClearAreaOfObjects(vectors.x, vectors.y, vectors.z, radius)
end)

RegisterNetEvent('ev-cleanup:clearAreaOfPeds')
AddEventHandler('ev-cleanup:clearAreaOfPeds', function (vectors, radius)
    ClearAreaOfPeds(vectors.x, vectors.y, vectors.z, radius)
end)

RegisterNetEvent('ev-cleanup:clearAreaOfVehicles')
AddEventHandler('ev-cleanup:clearAreaOfVehicles', function (vectors, radius)
