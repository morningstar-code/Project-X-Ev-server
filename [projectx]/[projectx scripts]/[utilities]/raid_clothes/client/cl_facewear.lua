...
function toggleFaceWear(pType, pRemove, pInfo, pSteal)
  ...
  elseif pType == "stolenshoes" then
    ...
    if currentDrawable ~= -1 and currentDrawable ~= bareFootIndex then
      SetPedComponentVariation(PlayerPedId(), PropIndex, bareFootIndex, 0, -1)
      TriggerServerEvent("ev-clothes:facewearSendItem", pSteal, pType, ItemMeta)
    end
  end
  ...
  if ItemHandler then
    ...
    if pSteal ~= false then
      ...
      if pRemove then
        print(json.encode(ItemMeta))
        TriggerEvent("player:receiveItem", pType, 1, true, ItemMeta)
      else
  ...
  TriggerEvent("ev-clothes:saveCurrentClothes")
...
AddEventHandler("ev-facewear:steal", function(pArgs)
...
AddEventHandler("ev-facewear:radial", function(pArgs)
  print("2")
  if antispam >= GetCloudTimeAsInt() then
    TriggerEvent("DoLongHudText", "Slow down okay?", 2)
    return
...
AddEventHandler("ev-facewear:clothesMenu", function(pArgs, pEntity)
  local context = {}
  local text = "Remove"
  local event = "ev-facewear:radial"

  if pArgs == "steal" then
    text = "Steal"
    event = "ev-facewear:steal"
  else
    pEntity = PlayerPedId()
  end
...
Citizen.CreateThread(function()
  ...
  exports["ev-interact"]:AddPeekEntryByEntityType(group, data, options)
end)