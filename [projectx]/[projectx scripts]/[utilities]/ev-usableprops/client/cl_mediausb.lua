AddEventHandler("ev-inventory:itemUsed", function(item, info, inventoryName, slot)
  ...
  if #results < 10 then
    context[#context + 1] = {
      title = "Add New Entry",
      action = "ev-usableprops:addUSBEntry",
      icon = "plus",
  ...
  for _, entry in pairs(results) do
    ...
    context[#context + 1] = {
      ...
      children = {
        { title = "View (opens in browser)", icon = "eye", description = url, action = "ev-usableprops:viewUSBEntry", key = url },
        {
          title = "Delete",
          icon = "trash",
          children = {
            {
              title = "Are you sure?",
              icon = "check-square",
              action = "ev-usableprops:deleteUSBEntry",
...
RegisterUICallback("ev-usableprops:addUSBEntry", function(pData, pCb)
  ...
  RPC.execute("ev-usableprops:saveUSB", data.meta.id, prompt.title, prompt.url)
  TriggerEvent("inventory:updateItem", data.item, data.slot, json.encode(data.meta));
end)

RegisterUICallback("ev-usableprops:viewUSBEntry", function(pData, pCb)
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  local url = pData.key
  TriggerEvent("ev-ui:openUrl", url)
end)

RegisterUICallback("ev-usableprops:deleteUSBEntry", function(pData, pCb)
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  local data = pData.key
