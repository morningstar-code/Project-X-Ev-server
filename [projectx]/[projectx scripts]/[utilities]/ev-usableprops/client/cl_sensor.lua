...
function SensorCreated(pObject, pEntity)
    ...
    if not polyzoneCreated[pObject.id] then
        ...
        exports['ev-polyzone']:AddBoxZone('movement_sensor', offset, pObject.data.metadata.length, 1.0, {
            data = { id = pObject.id },
...
RegisterNetEvent('ev-usableprops:toggleSensor', function(pId, pToggle)
...
AddEventHandler('ev-polyzone:enter', function(pZone, pData)
    ...
    if ActiveSensors[sensorId] and lastTrigger + 10000 < GetGameTimer() then
        ...
        RPC.execute('ev-usableprops:sensorTriggered', sensorId)
    end
end)

local lastHackAttempt = 0
AddEventHandler('ev-usableprops:hackSensor', function(pParameters, pEntity, pContext)
    ...
    local failed = false
    exports['ev-ui']:clearSkillCheck()
    for i=1,10 do
        local skillGap = math.random(5, 15)
        exports['ev-ui']:taskBarSkill(difficulty, skillGap, function(result)
            if result ~= 100 then
                ...
                if ActiveSensors[id] then
                    RPC.execute('ev-usableprops:sensorTriggered', id)
                end
                return
            end
        end, i % 2 == 0, true)
    ...
    TriggerEvent('ev-usableprops:disableSensor', id, 180000)
