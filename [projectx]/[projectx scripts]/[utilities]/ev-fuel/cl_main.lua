...
local function SetVehicleFuel(sentVeh, sentFuel)
...
local function GetVehicleFuel(sentVeh)
	if not DoesEntityExist(sentVeh) then return end;
...
local function doFuel(sentVeh)
	...
	if DoesEntityExist(sentVeh) then
		local currFuel = GetVehicleFuel(sentVeh)
		...
		elseif IsVehicleEngineOn(sentVeh) then
			if GetDisplayNameFromVehicleModel(model) == "savanna" then
				...
				SetVehicleFuel(sentVeh, currFuel - FuelUsage[Round(GetVehicleCurrentRpm(sentVeh), 1)] * (FuelClasses[GetVehicleClass(sentVeh)] or 1.0) / 10) -- From LegacyFuel
			end
		end
	end
end
...
exports('SetVehicleFuel', SetVehicleFuel) -- exports['ev-fuel']:SetVehicleFuel(veh, fuel)
exports('GetVehicleFuel', GetVehicleFuel)-- exports['ev-fuel']:GetVehicleFuel(veh)
...
RegisterUICallback("ev-fuel:FuelSelected", function(data, cb)
...
RegisterNetEvent('ev-fuel:RefillVehicle', function()
	...
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	...
	local menuData = {
		{
			...
			description = "Fuel Amount: ".. math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh)) .. " | Total Cost: $".. FuelCost .. ".0",
			icon = 'info-circle',
			key = "EVENTS.GAS",
		},
		{
		...
		},
		{
			...
			icon = 'address-card',
			key = "EVENTS.CARD",
			action = 'ev-fuel:selfServe',
			disabled = canPayBill --[[ not exports['ev-business']:IsEmployedAt('gofuel') ]]
		},
	}
	exports["ev-ui"]:showContextMenu(menuData)
end)

RegisterUICallback('ev-fuel:selfServe', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	...
	print(source)
	TriggerServerEvent("ev-phone:showGasPrice2", FuelCost)	
end)

RegisterUICallback('ev-fuel:pay_bank', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterUICallback('ev-fuel:pay_cash', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	local FuelCost = endFuel * GallonPrice
	SetUIFocus(false, false)
	TriggerServerEvent("ev-fuel:PurchaseSuccessfulCash", FuelCost)
end)

RegisterUICallback('ev-fuel:send_bill', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	...
	TriggerServerEvent("ev-phone:showGasPrice", data.values.pPaypalID, FuelCost)
end) 

RegisterUICallback('ev-fuel:send_billSelf', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
...
RegisterNetEvent('ev-fuel:OfferAcceptedCash', function()
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	local FuelCost = endFuel * GallonPrice
	if not paid then
		paid = true
		--TriggerServerEvent('ev-fuel:PurchaseSuccessful', FuelCost)
	end
	canRefillVehicle = false
	canPayBill = true
end)
 
RegisterNetEvent('ev-fuel:cl:phone:offer:accepted', function()
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
	local endFuel = (100 - vehicleCurrentFuel)
	local FuelCost = endFuel * GallonPrice
	print("TESTSTSTSTSTSTSST")
	if not paid then
		print("TESTSTS12312312312TSTSTSTSST")
		paid = true
...
RegisterUICallback('ev-fuel:RefuelVehicle', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
	local veh = getVehicleClosestToMe()
	local vehicleCurrentFuel = math.ceil(exports['ev-vehicles']:GetVehicleFuel(veh))
...
function refillVehicle()
  ...
  RegisterNetEvent("ev-ui:jerry_can:refuel")
  AddEventHandler("ev-ui:jerry_can:refuel",function()
