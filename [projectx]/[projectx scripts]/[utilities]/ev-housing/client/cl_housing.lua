...
function Housing.func.enterBuilding(propertyID, enterOverride, counterPart)
    ...
    --RPC.execute("ev-housing:halloween:playerEnteredProperty", propertyID)
    ...
    local isBuiltCoords,objects = exports["ev-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true, propertyID)
    ...
    if isBuiltCoords then
        ...
        if not counterPart then
            TriggerEvent("ev-editor:buildName", propertyID, objects, offsets)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
function placeBench(dropInventory)
    local buildingVector = exports["ev-build"]:getModule("func").currentBuildingVector()
    local hasBenches = exports["ev-config"]:GetMiscConfig("housing.benches")
...
function buildFurnitureData(propertyID)
    ...
    TriggerServerEvent("ev-housing:CheckFurniture", data, propertyID)
end
...
AddEventHandler('housing:crafting', function()
    ...
    if not hasCraftingProg then
        ...
        TriggerEvent("server-inventory-open", (serverCode and serverCode == 'pg') and '42096' or '42079', "Craft");
        return
    end
...
AddEventHandler('housing:toggleClosestLock', function()
    ...
    local jobAccess = exports["ev-business"]:IsEmployedAt("smol_dick_realtors")
