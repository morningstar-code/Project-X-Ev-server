...
RegisterNetEvent('ev-housing:editOffset')
AddEventHandler('ev-housing:editOffset', function(positionVec4)
    ...
    local shell, modelDim = exports["ev-build"]:getModule("func").GetShell(Housing.info[closest].model)
    ...
    Citizen.CreateThread(function()
        ...
        for i=1201, 1252 do
            if Housing.info[i].model ~= 'shop' and Housing.info[i].model ~= 'buisness_high'  then
                ...
                exports["ev-build"]:getModule("func").exitCurrentRoom(Housing.info[propertyID][1])
            end
        end
...
function spawnBuilding(propertyID, spawnBuildingLocation)
    ...
    local isBuiltCoords,objects = exports["ev-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,nil,nil,true)
...
function runCheck(propertyID,spawnBuildingLocation)
    ...
    local shell, modelDim = exports["ev-build"]:getModule("func").GetShell(Housing.info[propertyID].model)
    local boxPoints = {
