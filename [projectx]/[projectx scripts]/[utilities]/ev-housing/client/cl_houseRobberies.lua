...
Citizen.CreateThread(function()
    exports["ev-interact"]:AddPeekEntryByModel({ `prop_ld_int_safe_01` }, {
        {
...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if Housing.housingBeingRobbedClient[propertyID].hasBeenRobbed == false then
            ...
            for i=1,2 do
                if not failed then
                    local finished = exports["ev-ui"]:taskBarSkill(1650,  math.random(10, 14))
            ...
            Citizen.CreateThread(function()
                ...
                local baseProg = exports["ev-progression"]:GetProgression("streetrep") or 0
...
AddEventHandler("housing:exitFrontDoor", function()
    Housing.currentlyRobInside = false
    DoScreenFadeOut(1)
    exports["ev-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id][1])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
function displayPickup(force, itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["ev-ui"]:showInteraction("[E] Take Object")
        else
            exports["ev-ui"]:showInteraction("[E] Search location")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        ...
        local maxWeight = exports["ev-inventory"]:getPlayerMaxWeight()
    ...
    if type(item) == "string" then
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["ev-taskbar"]:taskBar(15000,"Searching Location")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            for k,v in pairs(itemTable) do
                ...
                TriggerServerEvent("ev-gallery:generateGem", "robbery")
...
AddEventHandler("ev-housing:alarmEnter", function()
    ...
    for i=1,2 do
        if not failedAlarm then
            local finished = exports["ev-ui"]:taskBarSkill(3200,  math.random(10, 14))
            if finished ~= 100 then
                failedAlarm = true
            end
        end
...
AddEventHandler("housing:takeSafe", function(pData, pEntity)
    ...
    local finished = exports['ev-taskbar']:taskBar(20000, "Picking up safe", true, true, nil, false);
