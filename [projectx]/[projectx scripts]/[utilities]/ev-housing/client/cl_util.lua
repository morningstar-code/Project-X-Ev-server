...
AddEventHandler('ev-housing:hotreload', function()
    defaultStart()
end)

RegisterCommand("housing:reload", function()
    defaultStart()
end)
...
RegisterUICallback("ev-housing:handler", function(data, cb)
...
AddEventHandler("property:menuAction", function(pData)
    ...
    if isComplete and dist <= 3.0 then
        ...
        elseif action == "forfeit" then
            ...
            exports["ev-ui"]:showContextMenu(MenuData["property_check"])
        end
    else
...
function hasPermissionInProperty()
    ...
    if exports['ev-business']:HasPermission(Housing.currentHousingInteractions.buisness, 'craft_access') then
        return true
    end
end

function getPropertyBuisness()
    if not Housing.currentlyInsideBuilding then
        return nil
    end

    return Housing.currentHousingInteractions.buisness
end
...
function hasRealtorAccess(checkHire)
    ...
    if checkHire then
        local realtor = exports['ev-business']:HasPermission('smol_dick_realtors', 'hire')
        local statecontracting = exports['ev-business']:HasPermission('statecontracting', 'hire')

        hasAccess = realtor or statecontracting
    else
        local realtor = exports["ev-business"]:IsEmployedAt("smol_dick_realtors")
        local statecontracting = exports["ev-business"]:IsEmployedAt("statecontracting")
