...
function polyHelperEnter(apartmentLocNumber, apartmentNumber)
    ...
    listening = true
    exports["ev-ui"]:showInteraction(getInteractionMessage(apartmentLocNumber, apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber, apartmentNumber)
    ...
    for k, v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = {title = "Apartment" .. " # " .. k, action = "ev-ui:apartmentsContext", key = {"EnterUnlocked", apartmentLocNumber, k}}
        end
    end
...
RegisterUICallback("ev-ui:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["ev-ui"]:showInteraction(getInteractionMessage(globalLocNumber, globalNumber))
    end
    ...
    if action == "lockdownCid" then
        ...
        Wait(10)
        local elements = {
            {name = "cid", label = "Player CID", icon = "fas fa-scroll"}
        }

        local prompt = exports['ev-ui']:OpenInputMenu(elements, function(values)
            return type(tonumber(values.cid)) == 'number' and tonumber(values.cid) >= 1
        end)
...
RegisterNetEvent('ev-apps:unlocks1')
AddEventHandler('ev-apps:unlocks1', function(roomNum)
    if globalLocNumber ~= nil then
        showApartmentMenu(globalLocNumber, roomNum)
    else
        TriggerEvent('DoLongHudText', "No Apartments are in use right now", 1)
    end
end)

RegisterNetEvent('ev-apps:unlocks2')
AddEventHandler('ev-apps:unlocks2', function()
    if globalLocNumber ~= nil then
        showApartmentMenu(globalLocNumber, globalNumber)
    else
        TriggerEvent('DoLongHudText', "No Apartments are in use right now", 1)
