...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState and not isHidden then
            exports["ev-ui"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess and not isHidden then
                        exports["ev-ui"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                    end
                else
                    idle = 100
            ...
            if currentDoorId ~= nil and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                if hasAccess then
                    local _, isAnimalModel = pcall(function () return exports["ev-misc-code"]:isAnimalModel(GetEntityModel(PlayerPedId())) end)
        ...
        exports["ev-ui"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
AddEventHandler("ev:target:changed", function(pEntity, pEntityType, pEntityCoords)
...
local function printDoorData()
    ...
    TriggerServerEvent("ev-doors:printDoorData", lastDoorData)
end

Citizen.CreateThread(function()
    if GetConvar('sv_environment', 'prod') == 'debug' then
        exports['ev-keybinds']:registerKeyMapping('', 'zzAdmin', 'Print Door Data', '+printDoorData', '-printDoorData', 'E');
...
RegisterCommand("KeyFobSpecial", function(pSrc, pArgs, pRaw)
        isSpecial = true
        TriggerEvent("DoLongHudText", "You have enabled the special keyfob sound.",2)
end, false)
...
AddEventHandler("ev-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local doorEntity = exports['ev-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent('ev-fx:sound:playGateLocking', isLocked, isSpecial)
...
Citizen.CreateThread(function()
    ...
    for _, door in pairs(DOOR_CONFIG) do
        TriggerEvent("ev-cleanup:addBlacklistedProp", door.model)
    end
end)
...
--     TriggerServerEvent("ev-doors:save-config", doorsCache)
-- end)
