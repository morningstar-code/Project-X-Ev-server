...
function OpenElevatorMenu(pElevator, pCurrentFloor, pBreachingDevice)
    ...
    exports['ev-ui']:showContextMenu(reverse(elements))
end
...
RegisterUICallback('ev-doors:elevator:teleport', function (data, cb)
    ...
    TriggerEvent("InteractSound_CL:PlayOnOne","elevator",0.2)
    TeleportPlayer(coords, heading, onArriveEvent, onLeaveEvent)
end)

RegisterUICallback('ev-doors:elevator:access', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterUICallback('ev-doors:elevator:hackControlPanel', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterUICallback('ev-doors:elevator:completeHacking', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
