...
RegisterNetEvent("ev-doors:mrpd:keyStatusUpdate")
AddEventHandler("ev-doors:mrpd:keyStatusUpdate", function()
    clearAccessCache()
end)
...
function hasSecuredAccess(pId, pType)
    ...
    local policeHasKeys = exports["ev-config"]:GetMiscConfig("police.masterkeys")
    ...
    local hasMrpdKey = RPC.execute("ev-doors:charHasMrpdKeys", characterId)
    ...
    local hackedAccess = RPC.execute('ev-doors:getHackedAccess')
