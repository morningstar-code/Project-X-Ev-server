...
function ConnectToDTFrequency(pIsHeadset)
    local subscribers = RPC.execute('ev-voice:drivethru:subscribe')
...
function DisconnectFromDTFrequency()
    ...
    RPC.execute('ev-voice:drivethru:unsubscribe')
    DTIsHeadset = false
...
function IncreaseDTRadioVolume()
    Debug("[DriveThru] Current Volume : %s", DTVolume)
...
function LoadDTModule()
    ...
    exports["ev-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Push-To-Talk", "+transmitToDriveThru", "-transmitToDriveThru")
    ...
    exports["ev-keybinds"]:registerKeyMapping("", "DriveThru Headset", "On / Off", "+toggleDTRadioState", "-toggleDTRadioState")
    ...
    exports["ev-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Up", "+increaseDTVolume", "-increaseDTVolume")
    RegisterCommand('+increaseDTVolume', IncreaseDTRadioVolume, false)
    RegisterCommand('-increaseDTVolume', function() end, false)

    exports["ev-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Down", "+decreaseDTRadioVolume", "-decreaseDTRadioVolume")
    ...
    RegisterNetEvent('ev-voice:drivethru:connect', ConnectToDTFrequency)

    RegisterNetEvent('ev-voice:drivethru:disconnect', DisconnectFromDTFrequency)

    RegisterNetEvent('ev-voice:drivethru:setPowerState', SetDTRadioPowerState)

    RegisterNetEvent('ev-voice:drivethru:addSubscriber', AddDTSubscriber)

    RegisterNetEvent('ev-voice:drivethru:removeSubscriber', RemoveDTSubscriber)

    -- scuffed way of overriding vanilla keybind
    AddEventHandler('ev:voice:transmissionStarted', function()
