...
AddEventHandler("mumbleConnected", function()
    ServerId = GetPlayerServerId(PlayerId())
    Debug("Mumble: Connected")
    TriggerEvent('ev:voice:state', true)
end)

AddEventHandler("mumbleDisconnected", function()
    Debug("Mumble: Disconnected")
    TriggerEvent('ev:voice:state', false)
end)

AddEventHandler('ev:voice:state', function (state)
    VoiceEnabled = state

    TriggerServerEvent("ev:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        if transmit then
            TriggerServerEvent("ev:voice:transmission:state", serverID, context, true)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("ev:voice:transmission:state", serverID, context, false)
        end
...
function AddGroupToTargetList(group, context, pIsInVehicle)
    ...
    TriggerServerEvent("ev:voice:transmission:state", group, context, true, nil, pIsInVehicle)
end
...
function RemoveGroupFromTargetList(group, context, transmit)
    ...
    if transmit then
        TriggerServerEvent("ev:voice:transmission:state", group, context, false)
    end
end
...
exports("GetVoiceProximity", GetVoiceProximity)
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("ev:voice:proximity", proximity)
...
AddEventHandler("ev:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
    ...
    local effect = exports['ev-galleria']:GetEffect('RADIO_STRENGTH')
    ...
    if context == "radio" and isCivRadio and transmitting then
        local getSendingRange = #(GetPlayerCoords(GetPlayerServerId(PlayerId())) - GetPlayerCoords(serverID))
...
RegisterNetEvent('ev:voice:targets:player:add')
AddEventHandler('ev:voice:targets:player:add', AddPlayerToTargetList)

RegisterNetEvent('ev:voice:targets:player:remove')
AddEventHandler('ev:voice:targets:player:remove', RemovePlayerFromTargetList)

RegisterNetEvent('ev:voice:refresh')
AddEventHandler('ev:voice:refresh', RefreshConnection)

RegisterNetEvent('ev:voice:proximity:override')
AddEventHandler('ev:voice:proximity:override', function(pId, pMode, pRange, pPriority)
...
AddEventHandler('ev-spawn:characterSpawned', RefreshConnection)
...
RegisterNetEvent('ev-voice:setTransmissionDisabled', function (pType, pDisabled)
...
Citizen.CreateThread(function()
    exports["ev-keybinds"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "Z")
    RegisterCommand('+cycleProximity', CycleVoiceProximity, false)
    RegisterCommand('-cycleProximity', function() end, false)

    exports["ev-keybinds"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
    ...
    TriggerEvent("ev:voice:ready")

    SetSettings(exports["ev-base"]:getModule("SettingsData"):getSettingsTable())
end)

RegisterNetEvent('event:settings:update')
AddEventHandler('event:settings:update', SetSettings)

RegisterNetEvent('ev-voice:setVoiceProximity')
AddEventHandler('ev-voice:setVoiceProximity', function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("ev:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("ev:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
