...
export async function InitEvents() {
    emitNet('ev-fiber:phoneAPIReady');
    await CPX.Events.start();
    await netSession();
}
...
on("ev-spawn:characterSpawned", /*548*/ () => {
    setTimeout((/*562*/) => {
        playerState.hasSpawned = true;
    }, 5000);
});

on("ev-base:spawnInitialized", /*582*/ () => {
...
onNet("ev-editor:loadEditor", /*603*/ () => {
    return {
...
onNet("ev-editor:closeEditor", /*623*/ () => {
    playerState.onEditor = false;
});

on("ev-fiber:hotreload", /*659*/ (a1) => {
