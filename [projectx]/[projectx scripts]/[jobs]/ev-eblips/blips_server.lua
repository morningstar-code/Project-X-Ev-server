...
RegisterNetEvent('ev-eblips:server:registerSourceName')
AddEventHandler('ev-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('ev-eblips:server:registerPlayerBlipGroup')
AddEventHandler('ev-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('ev-eblips:server:removePlayerBlipGroup')
AddEventHandler('ev-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('ev-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('ev-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('ev-eblips:server:registerPlayerBlipGroup', source, 'police')
    SetTimeout(time, function()
        TriggerEvent('ev-eblips:server:removePlayerBlipGroup', source, 'police')
    end)
end)
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('ev-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
