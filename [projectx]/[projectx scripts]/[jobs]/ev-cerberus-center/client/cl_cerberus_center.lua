...
AddEventHandler("ev-cbc:deleteBusiness", function()
  ...
  if #offices == 0 then return TriggerEvent("DoLongHudText", "No offices to delete.", 2) end
...
RegisterUICallback("ev-ui:cbc:deleteBusiness", function(pData, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ev-ui"]:closeApplication("textbox")
  RPC.execute("ev-cbc:deleteBusinessFromCenter", pData.values.cbc_id)
end)
...
RegisterUICallback('ev-cbc:actionOffice', function (data, cb)
  ...
  RPC.execute("ev-cbc:actionOffice", action, office, cid, currentJob == "police" or currentJob == "judge")
end)