...
RPC.register("ev-farmersmarket:claimBooth", function(pSource, pBoothId, pCharacterId, pPassword)
    print("asdadsad")
    ...
    TriggerClientEvent("ev-farmersmarket:updateBoothSettings", -1, boothSettings)
end)

RPC.register("ev-farmersmarket:leaveBooth", function(pSource, pBoothId, pCid)
...
RPC.register("ev-farmersmarket:joinBooth", function(pSource, pBoothId, pCharacterId, pPassword)
    if not boothSettings[pBoothId] then
        TriggerClientEvent("DoLongHudText", pSource, "Booth not claimed", 2)
        return
    end
...
RPC.register("ev-farmersmarket:isAtBooth", function(pSource, pBoothId, pCharacterId)
    if not boothSettings[pBoothId] then
        TriggerClientEvent("DoLongHudText", pSource, "Booth not claimed", 2)
        return
    end
...
RPC.register("ev-farmersmarket:getBoothCids", function(pSource, pBoothId)
    local src = pSource
    local cids = {}
    local contextTitle = ""
    if not boothSettings[pBoothId] then
    ...
    TriggerClientEvent("ev-ui:open-application", src, "contextmenu", {
        position = "right",
        options = cids
    }, true)
end)

RPC.register("ev-farmersmarket:getBoothSettings", function(pSource, pArea)
    local booths = {}
...
RPC.register("ev-farmersmarket:changeBoothImage", function(pSource, pBoothId, pCid, pId)
...
RPC.register("ev-farmersmarket:getCraftItem", function(pSource, pId)
    print("getCraftItem", pId)
end)

RPC.register("ev-farmersmarket:registerItem", function(pSource, pValues, pCid)
    print("registerItem", pValues, pCid)
end)

RPC.register("ev-farmersmarket:registerBanner", function(pSource, pValues, pCid)
    local src = source
    ...
    for _, v in pairs(boothSettings) do
        if (v.characterId == pCid) then
            ...
            --exports["ev-log"]:AddLog("Farmers Market", src, ("Banner ID: %s | Image: %s | Total Banners: %s"):format(bannerId, pValues.image, #registeredBanners))
...
RPC.register("ev-farmersmarket:getFarmersItems", function(pSource, pSearch)
    print("getFarmersItems", pSearch)
end)

RPC.register("ev-farmersmarket:farmersItemAction", function(pSource, pData)
