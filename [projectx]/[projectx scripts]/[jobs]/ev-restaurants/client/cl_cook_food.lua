...
FoodItems = CacheableMap(function (ctx, pRestaurant)
  local items = RPC.execute("ev-restaurants:getFoodItems", pRestaurant)

  if not items then return false, nil end

  return true, items
end, { timeToLive = 1 * 5 * 1000 })
...
AvailableMainDishes = CacheableMap(function (ctx, pRestaurant)
  local items = RPC.execute("ev-restaurants:fetchMenu", pRestaurant)
  if not items then return false, nil end
...
AddEventHandler("ev-restaurants:createFoodItem", function(pParams, p2, pContext)
  local data = pParams.isEditorPeek and pParams or pContext.zones["restaurant_create_food_item"]
  if pParams.isEditorPeek then
    data.restaurant = exports["ev-housing"]:getCurrentPropertyID()
  end
  ...
  for _, item in pairs(foodItems) do
    ...
    contextMenu[#contextMenu + 1] = {
      ...
      icon = "utensils",
      key = item,
      action = "ev-ui:restaurant:cookFoodItem",
...
RegisterUICallback("ev-ui:restaurant:cookFoodItem", function(data, cb)
  ...
  while true do
    if math.random() < 0.05 then
      local finished = exports["ev-ui"]:taskBarSkill(math.random(2000, 8000), math.random(7, 12))
    ...
    for _, item in pairs(items) do
      for _, ingred in pairs(uniqueIngredients) do
        if hasCategory(item.__og_food_cats and item.__og_food_cats or item.foodCategory, ingred) then
          ...
          local qty = exports["ev-inventory"]:getQuantity(item.itemName, true)
    ...
    for ingred, count in pairs(ingredCounts) do
      if currentIngredientTotals[ingred] < count then
        TriggerEvent("DoLongHudText", "Not enough ingredients!", 2)
        enoughIngredients = false
      end
    end
    if not enoughIngredients then break end
    local success = exports["ev-taskbar"]:taskBar(5000, "Preparing...", true, false, false, false, nil, 0.5, PlayerPedId())
    if success ~= 100 then break end
    local metaDatas = {}
    for ingred, items in pairs(currentIngredientCounts) do
      local processedCount = 0
      for itemName, count in pairs(items) do
        local itemData = exports["ev-inventory"]:getItemsOfType(itemName, count, true)
...
RegisterNetEvent("ev-restaurants:changedFoodItems", function(pRestaurant)
  resetFoodItems(pRestaurant)
end)

RegisterNetEvent("ev-restaurants:changedMenuItems", function(pRestaurant)
