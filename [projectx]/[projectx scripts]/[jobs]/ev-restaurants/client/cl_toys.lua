AddEventHandler('ev-restaurants:manageToyMenu', function(pParams, pEntity, pContext)
    ...
    local currentToys = RPC.execute('ev-restaurants:getCustomToys', restaurant)

    local context = {
        {
            title = 'Add New Toy',
            icon = 'plus',
            action = 'ev-restuarants:createToy',
            key = restaurant
        }
    }
            ...
            children = {
                {
                    title = 'Delete',
                    icon = 'trash',
                    action = 'ev-restuarants:removeToy',
                    key = {
...
RegisterUICallback("ev-restuarants:createToy", function(data, cb)
    ...
    local result = RPC.execute('ev-restaurants:createToyItem', data.key, prompt)
    if not result then
...
RegisterUICallback("ev-restuarants:removeToy", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local result = RPC.execute('ev-restaurants:removeToyItem', data.key.restaurant, data.key.id)
    if not result then
...
AddEventHandler('ev-restaurants:getCustomToyItem', function(pParameters, pEntity, pContext)
    local restaurant = pContext.zones['restaurant_takeout'].restaurant
    local toys = RPC.execute('ev-restaurants:getCustomToys', restaurant)
