...
local function reelInFish()
    ...
    for _, fish in pairs(fishes) do
        if not found and fish.chance > fishChance then
            ...
            if success then
                TriggerEvent('ev-fishing:caught_fish') -- SHIT
                TriggerServerEvent("ev-gallery:generateGem", "fishing")
...
AddEventHandler("ev-fishing:jobEvent", function(pActivityId, pTaskName, pReferences, pObjectives, pCallback)
