...
RegisterServerEvent("ev-business:server:pawnhub:getBalance")
AddEventHandler("ev-business:server:pawnhub:getBalance", function()
    local src = source
...
RegisterServerEvent("ev-business:server:pawnhub:startRun")
AddEventHandler("ev-business:server:pawnhub:startRun", function()
    local src = source
    ...
    currentDropoff = 1
    runTimer = 0
    TriggerEvent("ev-objects:prepareObject", "prop_cs_heist_bag_02", dropoffPos.x, dropoffPos.y, dropoffPos.z - 0.88, 5, metaData)
    TriggerClientEvent("ev-business:client:pawnhub:pickupLocationMarked", source, dropoffPos)
end)

RegisterServerEvent("ev-business:server:pawnhub:pickupCash")
AddEventHandler("ev-business:server:pawnhub:pickupCash", function(pId)
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)  
    local char = user:getCurrentCharacter()
    ...
    if runInProgress then
        TriggerEvent("ev-objects:DeleteObject", pId)
        ...
        isPickedup = true 
        dropoffPos = nil
        TriggerClientEvent("ev-business:client:pawnhub:completedRun", source)

        if currentDropoff > 4 then
            TriggerEvent("ev-business:server:pawnhub:endRun")
        end

        Citizen.Wait(10 * 60 * 1)
        TriggerEvent("ev-business:server:pawnhub:starNewtRun", src)
    end
end)

RPC.register("ev-inventory:sellStolenItems", function (src, pAccess)
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)  
    local inventory = exports["ev-inventory"]:getInventory("Stolen-Goods-PH-wl")
    
    if (pAccess) then
        for amount, v in pairs(inventory) do
            for k, h in ipairs(StolenGoods) do
                if v.item_id == h then
                    local qnt = exports["ev-inventory"]:getQuantitySvP(src, "Stolen-Goods-PH-wl", v.item_id, false, false)
                    local finalPrice = exports["ev-inventory"]:getItemPrice(v.item_id) * qnt
...
RegisterServerEvent("ev-business:server:pawnhub:starNewtRun")
AddEventHandler("ev-business:server:pawnhub:starNewtRun", function(src)
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)  
    local char = user:getCurrentCharacter()
    local z = 0
    ...
    TriggerEvent("ev-objects:prepareObject", "prop_cs_heist_bag_02", dropoffPos.x, dropoffPos.y, z, 5, metaData)
    TriggerClientEvent("ev-business:client:pawnhub:pickupLocationMarked", src, dropoffPos)
end)

RegisterServerEvent("ev-business:server:pawnhub:endRun")
AddEventHandler("ev-business:server:pawnhub:endRun", function()
    if not runInProgress then
        TriggerClientEvent("DoLongHudText", source, "No run in progress.", 2)
        return
    end
