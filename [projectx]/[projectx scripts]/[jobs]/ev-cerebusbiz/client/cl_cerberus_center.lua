...
RegisterNetEvent("ev-jobmanager:playerBecameJob", function(job)
  currentJob = job
end)
...
function setIpls(pNumber)
  local officeID = tonumber(pNumber)
  local ipl = ipls[officeID][1]
  local intId = ipls[officeID][2]
  RequestIpl(ipl)
  if officeID ~= 4 then
...
function gotoOffice(pOffice, pBucketString)
  local finished = exports["ev-taskbar"]:taskBar(3000, "Waiting for Elevator")
  ...
  clearIpls()
  setIpls(pOffice)
  RPC.execute("ev-infinity:setWorld", bucketString .. pOffice, "inactive", true)
...
function leaveOffice()
  ...
  RPC.execute("ev-infinity:setWorld", "default")
  Wait(400)
  DoScreenFadeIn(1000)
end

RegisterNetEvent("ev-cbc:office", function(pOffice, pBucketString)
  gotoOffice(pOffice, pBucketString)
end)
RegisterNetEvent("ev-cbc:officeLeave", function()
  leaveOffice()
end)

AddEventHandler("ev-cbc:showElevatorOptions", function()
  exports['ev-ui']:showContextMenu({
    {
      title = "Lock Offices",
      description = "Lock Down All Offices",
      action = "ev-cbc:doContextAction",
      key = { type = "lock" },
    },
    {
...
RegisterUICallback('ev-cbc:doContextAction', function (data, cb)
  ...
  TriggerServerEvent("ev-cbc:RhodoOfficeInformation", data.key)
  Citizen.Wait(1350)
  RPC.execute("ev-cbc:doElevatorAction", data.key) 
end)

AddEventHandler("ev-cbc:addBusiness", function()
  ...
  local creationMenu = {
    key = 1,
    show = true,
    callbackUrl = "ev-ui:cbc:addBusiness",
    items = { },
  }
...
RegisterUICallback("ev-ui:cbc:addBusiness", function(pData, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ev-ui"]:closeApplication("textbox")
  TriggerServerEvent("ev-cbc:addBusinessToCenterRhodo", pData.values)
  RPC.execute("ev-cbc:addBusinessToCenter", pData.values)
end)

AddEventHandler("ev-cbc:deleteBusiness", function()
  local isEmployee = RPC.execute("IsEmployedAtBusiness", {
    character = {
      id = exports['isPed']:isPed('cid'),
    },
  ...
  local creationMenu = {
    key = 1,
    show = true,
    callbackUrl = "ev-ui:cbc:deleteBusiness",
    items = { },
  }
  local offices = RPC.execute("ev-cbc:getCreatedOffices")
  local officeOpts = {}
...
RegisterUICallback("ev-ui:cbc:deleteBusiness", function(pData, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ev-ui"]:closeApplication("textbox")
  raggsyy = pData.values.cbc_id
  TriggerServerEvent("ev-cbc:deleteBusinessFromCenter", raggsyy)
  RPC.execute("ev-cbc:deleteBusinessFromCenter", raggsyy)
end)

AddEventHandler("ev-cbc:showBusinessElevatorOptions", function()
  local offices = RPC.execute("ev-cbc:getCreatedOffices")
  local AE = {
    title = "A-E",
    children = {},
  ...
  exports['ev-ui']:showContextMenu(ctx)
end)

RegisterUICallback('ev-cbc:actionOffice', function (data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local action = data.key.action
  local office = data.key.office
  local cid = exports["isPed"]:isPed("cid")
  TriggerServerEvent("ev-cbc:actionOfficeInfo", action, office.offices_business_id, cid, currentJob == "police" or currentJob == "judge")
  RPC.execute("ev-cbc:actionOffice", action, office.offices_business_id, cid, currentJob == "police" or currentJob == "judge")
end)
