...
Citizen.CreateThread(function()
    exports["ev-polyzone"]:AddBoxZone("ev-distillery:tavern", vector3(2463.95, 3446.46, 50.11), 7.0, 2.2, {
        minZ = 44.91,
        maxZ = 51.31,
        heading = 5
    })
...
local function stageProcess(pActionText, pTaskTime, pTaskText, pServerEvent, pCanExplode)
  ...
  if IsControlJustReleased(1, 38) then
      ...
      local finished = exports["ev-taskbar"]:taskBar(pTaskTime, pTaskText, false, false, nil)
          ...
          if distilleryStatus then
                      ...
                      for _, mashType in pairs(distilleryStatus.mash) do
                          if mashType.count < batchRequirements[_].count then
                              if batchRequirements[_].validIngredient ~= nil then
                                  if exports["ev-inventory"]:hasEnoughOfItem(batchRequirements[_].validIngredient, 1,
                                      false) then
                                  ...
                                  if batchRequirements[_].validIngredients ~= nil then
                                      ...
                                      for __, validIngredient in pairs(batchRequirements[_].validIngredients) do
                                          if exports["ev-inventory"]:hasEnoughOfItem(validIngredient, 1, false) then
                                              TriggerServerEvent("distillery:addIngredient", _)
              ...
              elseif distilleryStatus.stage.current == 4 then
                  if not distilleryStatus.stage.ruined and not awaitingResponse then
                      ...
                      if IsControlJustReleased(1, 38) then
                          if exports["ev-inventory"]:hasEnoughOfItem("glass", 1, false) then
                              TriggerEvent("animation:PlayAnimation", "layspike")
                              local finished = exports["ev-taskbar"]:taskBar(1000, "Bottling moonshine", false, false,
                                  nil)
                              ClearPedSecondaryTask(PlayerPedId())
                              if (finished == 100) then
                                  awaitingResponse = true
...
AddEventHandler("ev-polyzone:enter", function(name, data)
    if name ~= "ev-distillery:tavern" then
        return
    end
...
RegisterNetEvent("ev-distillery:fruitShop")
AddEventHandler("ev-distillery:fruitShop", function()
