...
CreateThread(function ()
    exports["ev-polyzone"]:AddBoxZone("reed_pickup", vector3(1001.51, -2116.59, 30.48), 3.5, 3.5, {
        heading=357,
        minZ=29.48,
        maxZ=35.00,
        data = {
...
AddEventHandler("ev-polyzone:enter", function(pName, pData)
    if pName ~= "reed_pickup" then return end
    isInsideHookZone = true
end)

AddEventHandler("ev-polyzone:exit", function(pName, pData)
    if pName ~= "reed_pickup" then return end
    isInsideHookZone = false
end)

AddEventHandler("ev-miscscripts:reedHook", function(pParameters, pEntity, pContext)
    local hookedTargetServerId = GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity))
    if hookedTargetServerId ~= 0 then
        CPX.Events.emitNet('ev-miscscripts:reedSetHook', hookedTargetServerId)
    end
end)
...
CPX.Events.onNet('ev-miscscripts:reedSetHooked', function(pTarget, pState)
    print(pTarget, pState)
    if pState and not isHooked then
        CPX.Streaming.loadAnim(animDict)
        isHooked = true
...
RegisterCommand("-unhook", function()
    if isHooked or isInsideHookZone then
        CPX.Events.emitNet('ev-miscscripts:reedSetHook')
    end
end, false)