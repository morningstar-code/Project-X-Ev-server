...
RPC.register("ev-car-clothing:swapCurrentOutfit", function(src, pNetId, data)
...
RPC.register("ev-car-clothing:saveCurrentOutfit", function(src, pNetId, data)
    local entity = NetworkGetEntityFromNetworkId(pNetId)
    local currApp = exports["ev-vehicles"]:GetVehicleAppearance(pNetId)
    local currMods = exports["ev-vehicles"]:GetVehicleMods(pNetId)
    local vehData = Entity(tonumber(entity))
    local vin = vehData.state.vin
    local temp = {}
