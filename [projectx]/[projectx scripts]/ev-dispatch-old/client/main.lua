...
AddEventHandler("dispatch:get-police-cl", function(data, data2)
    police_table = data
    dispatch_table = data2
end)

RegisterNetEvent("dispatch:get-html")
AddEventHandler("dispatch:get-html", function(data)
    html = data
    SendNUIMessage({
        action = "update-html",
        code = html
...
AddEventHandler("dispatch:delete-police_cl", function(data)
    SendNUIMessage({
        action = "deletedata",
        player_code = data
    })
...
function callsign_command(callsign, pType)
    if playercode == nil then 
        callSing(callsign, pType)
    else
...
function callSing(code, icon)
    ...
    RPC.execute("ev-jobs:setCallsign", cid, job , code, icon)
    Citizen.Wait(550)
...
RegisterNetEvent("ev-dispatch:AddinSignin", function(code, icon)
    callSing(code, icon)
end)
...
AddEventHandler("dispatch:trigger-dispatch-client:normal", function(code, code1, name, street, alertcoordx, alertcoordy, time)
    ...
    if job == "police" then 
        ...
        TriggerServerEvent("ev-fx:sound:playDispatch", "HighPrioCrime")
        SendNUIMessage({
            action = "add-dispatch",
            mod = "normal",
            alertname = name,
...
function dispatch(code, alertName, plate, alertCoord, vehicle)
    ...
    if IsPedInAnyVehicle(playerPed, false) and vehActive then
        ...
        local color1 = colors[GetVehicleCustomPrimaryColour(vehicle)]
        local color2 = colors[GetVehicleCustomSecondaryColour(vehicle)]
        local plate = "Unknown"
...
function policedead(code, alertName, icon)
    ...
    TriggerServerEvent("ev-fx:sound:playDispatch", "10-1314")
...
function mapblip(alertName, x, y)
    if string.find(alertName, "Shots Fired" or "Gun Shot") then
        osoruk(x, y, 433, alertName)
    elseif string.find(alertName, "Bobcat Robbery") then
        osoruk(x, y, 110, alertName)
    elseif string.find(alertName, "Pacific Bank Robbery") then
        osoruk(x, y, 272, alertName)
    elseif string.find(alertName, "House Robbery") then
        osoruk(x, y, 40, alertName)
    elseif string.find(alertName, "PowerPlant Atack") then
        osoruk(x, y, 354, alertName)
    elseif string.find(alertName, "Fleeca Bank Robbery") then
        osoruk(x, y, 272, alertName)
    elseif string.find(alertName, "Paleto Bank Robbery") then
        osoruk(x, y, 272, alertName)
    elseif string.find(alertName, "Jewelry Robbery") then
        osoruk(x, y, 617, alertName)
    elseif string.find(alertName, "Store Robbery") then
        osoruk(x, y, 52, alertName)
    elseif string.find(alertName, "Suspicious Activity") then
        osoruk(x, y, 456, alertName)
    elseif string.find(alertName, "Drug Trafficking") then
        osoruk(x, y, 514, alertName)
    elseif string.find(alertName, "Illegal Substance Use") then
        osoruk(x, y, 788, alertName)
    elseif string.find(alertName, "Car Crash") then
        osoruk(x, y, 380, alertName)
    elseif alertName == "Car Crash" then
        osoruk(x, y, 429, alertName)
    elseif string.find(alertName, "Street Fight") then
        osoruk(x, y, 311, alertName)
    elseif string.find(alertName, "Deadly Weapon") then
        osoruk(x, y, 150, alertName)
    elseif string.find(alertName, "Bank Truck Robbery") then
        osoruk(x, y, 67, alertName)
    elseif alertName == "Stealing Vehicle" then
    elseif string.find(alertName, "Bank Truck Robbery") then
        osoruk(x, y, 225, alertName)
    elseif string.find(alertName, "Illegal Hunting") then
        osoruk(x, y, 463, alertName)
    end
end
...
if GetConvar("sv_environment", "prod") == "debug" then
    RegisterCommand('sound', function(source, args, RawCommand)
        TriggerServerEvent('ev-fx:sound:playDispatch', args[1])
    end)
end