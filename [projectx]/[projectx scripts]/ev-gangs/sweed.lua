...
AddEventHandler("weed:createplant", function(x,y,z, seed)
    ...
    exports.oxmysql:execute("INSERT INTO weed_plants (coords, seed, owner) VALUE (@coord, @seed, @owner)", {
...
AddEventHandler("weed:UpdateWeedGrowth", function(dbId, new)
	exports.oxmysql:Execute("UPDATE weed_plants SET growth = @new WHERE id = @db",{
        ['growth'] = new, 
        ['db'] = dbId
    })
end)
...
AddEventHandler("weed:requestTable", function()
    exports.oxmysql:execute('SELECT * FROM weed_plants', {}, function(result)
