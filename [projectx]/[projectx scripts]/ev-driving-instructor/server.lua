...
AddEventHandler('driving:submitTest', function(data)
  ...
  exports.oxmysql:execute("INSERT INTO driving_tests (cid, icid, instructor, timestamp, points, passed, results) VALUES (@cid, @icid, @instructor, @timestamp, @points, @passed, @results);",
  {['cid'] = drivingTests.cid, ['icid'] = drivingTest.icid, ['instructor'] = drivingTest.instructor, ['timestamp'] = drivingTest.timestamp, ['points'] = drivingTest.points, ['points'] = drivingTest.points, ['results'] = drivingTest.results}, function(rowsChanged)
    if (rowsChanged and rowsChanged ~= 0) then
      exports.oxmysql:execute("SELECT id FROM driving_tests WHERE cid = @cid AND icid = @icid AND timestamp = @timestamp",
...
AddEventHandler('driving:getHistory', function(source, cid)
  ...
  exports.oxmysql:execute("SELECT * FROM driving_tests WHERE cid = @cid ORDER BY id DESC LIMIT 5", {['cid'] = tonumber(cid)}, function(response)
    if (#response <= 0) then
...
AddEventHandler('driving:getReport', function(source, tID)
  ...
  exports.oxmysql:execute("SELECT * FROM driving_tests WHERE id = @testID", {['testID'] = tonumber(testID)}, function(response)
    if response[1] == nil then
