...
local function removeGuns()
  local qty = exports["ev-inventory"]:getQuantity("-2009644972")
  ...
  local megaphoneQty = exports["ev-inventory"]:getQuantity("megaphone", true, { type = "paintball" })
  if megaphoneQty and megaphoneQty > 0 then
    TriggerEvent("inventory:removeItemByMetaKV", "megaphone", megaphoneQty, "type", "paintball")
  end
  local smokeQty = exports["ev-inventory"]:getQuantity("smokegrenadenpa")
  if smokeQty and smokeQty > 0 then
    TriggerEvent("inventory:removeItem", "smokegrenadenpa", smokeQty)
  end
end

AddEventHandler("ev-paintball:getPaintballGun", function()
  TriggerEvent("player:receiveItem", "-2009644972", 1)
end)

AddEventHandler("ev-paintball:getCaddy", function()
  if not isNpa() then return end
  RPC.execute("ev:vehicles:basicSpawn", "caddy", { x = 5513.32, y = 5992.66, z = 590.01 }, 359.1, "paintball")
end)

AddEventHandler("ev-paintball:getGoKart", function()
  if not isNpa() then return end
  RPC.execute("ev:vehicles:basicSpawn", "kart", { x = 5513.32, y = 5992.66, z = 590.01 }, 359.1, "paintball")
end)

AddEventHandler("ev-paintball:getPaintballSmoke", function()
  if not isNpa() then return end
  TriggerEvent("player:receiveItem", "smokegrenadenpa", 1)
end)

AddEventHandler("ev-paintball:getPaintballAmmo", function()
  TriggerEvent("player:receiveItem", "paintballs", 20)
end)

AddEventHandler("ev-paintball:getMegaphone", function()
  if not isNpa() then return end
...
RegisterNetEvent("ev-paintball:reviveAfterDown", function()
...
AddEventHandler("ev-paintball:setArenaType", function(pArgs)
  if not isNpa() then return end
  RPC.execute("ev-paintball:setArenaType", pArgs[1])
end)

RegisterNetEvent("ev-paintball:changeArenaType", function(pType)
...
Citizen.CreateThread(function()
  Citizen.Wait(30000)
  TriggerServerEvent("ev-paintball:getArenaType")
end)
...
AddEventHandler("ev-paintball:swapLocations", function()
  DoScreenFadeOut(400)
...
AddEventHandler("ev-paintball:enterArena", function()
  inArena = true
  TriggerEvent("ev-cleanup:enableCleanup", false)
  Citizen.CreateThread(function()
    while inArena do
      if currentLocation == "arena" then
...
AddEventHandler("ev-paintball:exitArena", function()
  inArena = false
  TriggerEvent("ev-cleanup:enableCleanup", true)
  removeGuns()
end)
