...
function spawnBall()
  ...
  TriggerServerEvent("ev-deanworld:rocketLeagueBallId", NetworkGetNetworkIdFromEntity(ballObj))
end

function spawnCar(goal, spawnIndex, color)
  print(goal, spawnIndex, color)
...
function joinGame(pGoal, pIndex)
  ...
  if not IS_DEAN_WORLD_OPEN then
    TriggerEvent("DoLongHudText", "Dean World is closed!", 2)
    return
  end
  local teamColors = RPC.execute("ev-deanworld:rocketLeagueJoinGame", pGoal, pIndex)
  if not teamColors then return end
  inGame = true
  inGameTeam = pGoal
  inGameIndex = pIndex
  inGameTeamColor = teamColors
...
function leaveGame()
  ...
  RPC.execute("ev-deanworld:rocketLeagueLeaveGame", inGameTeam, inGameIndex)
...
RegisterNetEvent("ev-deanworld:rocketLeagueSetBallId")
AddEventHandler("ev-deanworld:rocketLeagueSetBallId", function(pNetId)
  if not inGame then return end
  Citizen.CreateThread(function()
    ...
    while ballObj and ballObj ~= 0 and continueBallLoop do
      ...
      if northGoal:isPointInside(ballCoords) then
        TriggerServerEvent("ev-deanworld:rocketLeagueGoalScored", "north", pNetId)
      elseif southGoal:isPointInside(ballCoords) then
        TriggerServerEvent("ev-deanworld:rocketLeagueGoalScored", "south", pNetId)
      end
      Citizen.Wait(250)
    end
  end)
...
RegisterNetEvent("ev-deanworld:rocketLeagueStats")
AddEventHandler("ev-deanworld:rocketLeagueStats", function(stats)
  if not inSoccerPitch then return end
  local titleExtra = ""
  local values = {}
  if not stats.gameInProgress then
    titleExtra = " - " .. "Ready for players!"
    values[#values + 1] = "Blue Team" .. ": " .. stats.bluePlayers .. " " .. "players"
    values[#values + 1] = "Red Team" .. ": " .. stats.redPlayers .. " " .. "players"
  else
    titleExtra = " - " .. "Game in progress!"
    values[#values + 1] = {
      center = true,
      text = "Blue" .. " " .. stats.blueScore .. " - " .. stats.redScore .. " " .. "Red"
    }
    values[#values + 1] = {
      center = true,
  ...
  exports["ev-ui"]:sendAppEvent("status-hud", {
    show = true,
    values = values,
    dealers = "Bumper Ball" .. titleExtra,
  })
end)

RegisterNetEvent("ev-deanworld:rocketLeagueFinishGame")
AddEventHandler("ev-deanworld:rocketLeagueFinishGame", function(blueScore, redScore)
  leaveGame()
  if not inSoccerPitch then return end
  TriggerEvent("DoLongHudText", "Final score" .. ": " .. "Blue" .. " " .. tostring(blueScore) .. " - " .. tostring(redScore) .. " " .. "Red", 1, 12000, { i18n = { "Final score", "Blue", "Red" }})
end)

RegisterNetEvent("ev-deanworld:rocketLeagueSpawnBall")
AddEventHandler("ev-deanworld:rocketLeagueSpawnBall", function(pSource)
  continueBallLoop = false
  if not inGame then return end
...
RegisterNetEvent("ev-deanworld:rocketLeagueGameStartCountdown")
AddEventHandler("ev-deanworld:rocketLeagueGameStartCountdown", function(pSeconds)
  if not inSoccerPitch then return end
  Citizen.CreateThread(function()
    local lc = pSeconds
    while lc > 0 do
...
AddEventHandler("ev-deanworld:rocketLeagueJoinGame", function(a, b, c)
...
AddEventHandler("ev-polyzone:enter", function(name)
  ...
  inSoccerPitch = true
  RPC.execute("ev-deanworld:rocketLeagueGetStats")
end)
...
AddEventHandler("ev-deanworld:purchaseRLUpgrades", function()
  TriggerEvent("DoLongHudText", "Jump / Boost / Cosmetic upgrades coming soon!")
end)

RegisterUICallback("ev-ui:rl:changePlayerCount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  TriggerServerEvent("ev-deanworld:rl:changePlayerCount", tonumber(data.key))
end)

AddEventHandler("ev-deanworld:rocketLeagueChangePlayers", function()
  local data = {}
  data[#data + 1] = {
    title = "1 vs 1",
    description = "",
    key = "1",
    action = "ev-ui:rl:changePlayerCount",
  }
  data[#data + 1] = {
    title = "2 vs 2",
    description = "",
...
-- polyzones
Citizen.CreateThread(function()
  exports["ev-polyzone"]:AddBoxZone("rc_soccer_pitch", vector3(-1624.08, -1084.72, 13.24), 21.2, 29.2, {
    heading=230,
    minZ=12.04,
    maxZ=16.64,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("rc_soccer_entry_pad", vector3(-1623.89, -1097.76, 13.12), 0.8, 0.6, {
    heading=320,
    minZ=13.12,
    maxZ=13.52,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("rc_soccer_entry_pad", vector3(-1622.13, -1095.9, 13.32), 0.8, 0.6, {
    heading=320,
    minZ=13.12,
    maxZ=13.52,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("rc_soccer_entry_pad", vector3(-1620.64, -1093.81, 13.32), 0.8, 0.6, {
    heading=320,
    minZ=13.12,
    maxZ=13.52,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("rc_soccer_entry_pad", vector3(-1614.46, -1086.65, 13.32), 0.8, 0.6, {
    heading=320,
    minZ=13.12,
    maxZ=13.52,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("rc_soccer_entry_pad", vector3(-1612.78, -1084.66, 13.32), 0.8, 0.6, {
    heading=320,
    minZ=13.12,
    maxZ=13.52,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("rc_soccer_entry_pad", vector3(-1611.15, -1082.62, 13.32), 0.8, 0.6, {
    heading=320,
    minZ=13.12,
    maxZ=13.52,
    data = {
  ...
  exports['ev-interact']:AddPeekEntryByPolyTarget('rc_soccer_entry_pad', {{
    event = "ev-deanworld:rocketLeagueJoinGame",
    id = "rljoingame",
    icon = "fas fa-play",
    label = "Join game!",
    parameters = {},
  ...
  exports['ev-interact']:AddPeekEntryByPolyTarget('rc_soccer_entry_pad', {{
    event = "ev-deanworld:rocketLeagueChangePlayers",
    id = "rlchangeplayers",
    icon = "fas fa-circle",
