...
Citizen.CreateThread(function()
  exports['ev-density']:RegisterDensityReason('deanworld', 10)
...
RegisterNetEvent("ev-deanworld:isOpen")
AddEventHandler("ev-deanworld:isOpen", function(isOpen)
  IS_DEAN_WORLD_OPEN = isOpen
end)

RegisterNetEvent("ev-deanworld:activateLasers")
AddEventHandler("ev-deanworld:activateLasers", function(doActivate)
...
AddEventHandler("ev-polyzone:enter", function(zone, data)
  ...
  IS_DEAN_WORLD_OPEN = RPC.execute("ev-deanworld:isDWOpen")
  exports['ev-density']:ChangeDensity('deanworld', 0.0)
  inDeanWorld = true
  dwDui = exports["ev-lib"]:getDui(screenUrl, 1024, 1024)
...
AddEventHandler("ev-polyzone:exit", function(zone, data)
  if zone ~= "farmers_market" then return end
  exports['ev-density']:ChangeDensity('deanworld', -1)
  inDeanWorld = false
  if dwDui ~= nil then
    RemoveReplaceTexture("dw_stage_txd", "dw_stage_screen_tx")
    exports["ev-lib"]:releaseDui(dwDui.id)--, "dw_stage_txd", "dw_stage_screen_tx")
  end
end)

RegisterNetEvent("ev-deanworld:changeScreenImage")
AddEventHandler("ev-deanworld:changeScreenImage", function(url)
  if inDeanWorld and dwDui ~= nil then
    exports["ev-lib"]:changeDuiUrl(dwDui.id, url)
  end
  screenUrl = url
end)
...
AddEventHandler("ev-deanworld:findLostStuff", function(zone, data)
  local charId = exports["isPed"]:isPed("cid")
  if not pickupCids[charId] then
    TriggerEvent("DoLongHudText", "Nothing found.", 2)
    return
  end
  local hasCash = RPC.execute("ev-deanworld:getOwedCash")
  exports["ev-ui"]:sendAppEvent("game", { getDeanWorldCsv = true })
...
AddEventHandler("ev-deanworld:buyShitFood", function(zone, data)
  TriggerEvent("server-inventory-open", "42075", "Shop")
end)

AddEventHandler("ev-deanworld:dropOffCases", function(zone, pEntity, p3)
  local charId = exports["isPed"]:isPed("cid")
  local result = RPC.execute("ev-deanworld:dropOffCases", charId)
...
AddEventHandler("ev-deanworld:buyAdministrationTicket", function(zone, data)
...
-- college field stuff
Citizen.CreateThread(function()
  exports["ev-polyzone"]:AddBoxZone("collegefieldscreen", vector3(-1744.18, 171.33, 64.37), 25, 25, {
    heading=305,
    minZ=62.77,
    maxZ=66.97,
    data = {
...
AddEventHandler("ev-polyzone:enter", function(zone, data)
  ...
  inCollegeField = true
  collegeFieldDui = exports["ev-lib"]:getDui(collgeFieldScreenUrl, 1024, 1024)
...
AddEventHandler("ev-polyzone:exit", function(zone, data)
  ...
  if collegeFieldDui ~= nil then
    RemoveReplaceTexture("dw_stage_txd", "dw_stage_screen_tx")
    exports["ev-lib"]:releaseDui(collegeFieldDui.id)
  end
end)

RegisterNetEvent("ev-deanworld:changeCollegeScreenImage")
AddEventHandler("ev-deanworld:changeCollegeScreenImage", function(pUrl)
  if inCollegeField and collegeFieldDui ~= nil then
    exports["ev-lib"]:swapDuiUrl(collegeFieldDui.id, pUrl)
  end
