...
Citizen.CreateThread(function()
  exports["ev-keybinds"]:registerKeyMapping("", "Player", "Crouch", "+moveCrouch", "-moveCrouch", "X")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if ctrlStage ~= 0 then
            if IsPedArmed(playerPed, 6) or IsPedJacking(playerPed) or IsPedInMeleeCombat(playerPed) or IsControlJustReleased(1,22) or IsPedRagdoll(playerPed) or handCuffed or handCuffedWalking or imdead == 1 or ( IsPedSittingInAnyVehicle( GetPlayerPed( -1 ) ) ) then
...
AddEventHandler('ev-police:cuffs:state', function(handCuffedSent,WalkingSent)
...
CreateThread(function()
    while true do
        ...
        if IsPedArmed(ped, 6) then
            local curw = GetSelectedPedWeapon(ped)
            local change = recoils[curw]
            if change then
