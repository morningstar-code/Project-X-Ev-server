...
AddEventHandler('ev-hasino:rappelDownElevator', function(pArgs, pEntity, pContext)
    -- todo: check for minivault rope item again
    local hasGrapple = exports['ev-inventory']:hasEnoughOfItem('grapplegun', 1, false, true)
        or exports['ev-inventory']:hasEnoughOfItem('grapplegunpd', 1, false, true)
    ...
    local finished = exports['ev-taskbar']:taskBar(5000, 'Attaching Rope')
...
function rappelDownWall(pCoords, pHeading, pPed, pRopeId)
    ...
    if pPed == PlayerPedId() then
        ...
        removeRope(1)
        ClearPedTasks(pPed)
        if exports['ev-inventory']:hasEnoughOfItem('grapplegun', 1, false) then
            TriggerEvent("inventory:DegenItemType",100,"grapplegun")
        elseif exports['ev-inventory']:hasEnoughOfItem('grapplegunpd', 1, false) then
...
AddEventHandler("ev-hasino:elevatorForceOpen", function(p1, p2, p3)
    local id = p3.zones.hasino_elevator_force_open.id
    local cfg = RPC.execute("ev-hasino:getElevatorConfig", id)
    local hasCrowbar = exports['ev-inventory']:hasEnoughOfItem('2227010557', 1, false, true)
    ...
    local finished = exports["ev-taskbar"]:taskBar(20000, "Forcing Open")
    ClearPedTasks(PlayerPedId())
    if finished ~= 100 then return end
    TriggerServerEvent("ev-hasino:elevatorForcedOpen", id)
end)
AddEventHandler("ev-hasino:elevatorDownForceOpen", function()
    TriggerServerEvent("ev-hasino:serverElevatorDownForceOpen")
end)
