local defaultLockBoxModel = GetHashKey('ch_prop_ch_sec_cabinet_01j')
local replaceLockBoxModel = GetHashKey('ch_prop_ch_sec_cabinet_01h')
...
RegisterNetEvent('ev-hasino:hideModel', function(box, coords, model)
...
RegisterNetEvent('ev-hasino:createBags', function()
...
--     TriggerEvent('ev-hasino:createBags')
-- end)

Citizen.CreateThread(function()
    exports["ev-interact"]:AddPeekEntryByModel({ replaceLockBoxModel }, {{
        event = "ev-hasino:robLockBox",
        id = "ev-hasino:robLockBox",
        icon = "hammer",
        label = "Rob!",
        parameters = {},
    }}, {
    ...
    exports["ev-interact"]:AddPeekEntryByModel({ keypadModel }, {{
        event = "ev-hasino:enterLVKeypad",
        id = "ev-hasino:enterLVKeypad",
        icon = "calculator",
        label = "Enter Input",
        parameters = {},
    }}, {
    ...
    exports["ev-interact"]:AddPeekEntryByModel({ drillBagHash }, {{
        event = "ev-hasino:takeHeistBag",
        id = "ev-hasino:takeHeistBag",
        icon = "hand-holding",
        label = "Grab",
        parameters = {},
    }}, {
...
AddEventHandler('ev-hasino:robLockBox', function(pArgs, pEntity, pContext)
    if GetEntityModel(pEntity) ~= replaceLockBoxModel or NetworkGetEntityIsNetworked(pEntity) ~= 1 then
        return
    end
    local hasBag = exports['ev-inventory']:hasEnoughOfItem('heistduffelbag', 1, false, true)
    if not hasBag then
        TriggerEvent('DoLongHudText', 'Missing duffel bag', 2)
        return
    end
    local netId = NetworkGetNetworkIdFromEntity(pEntity)
    TriggerServerEvent('ev-hasino:robLockBox', netId)
end)

AddEventHandler('ev-hasino:enterLVKeypad', function(pArgs, pEntity, pContext)
    local coords = GetEntityCoords(pEntity)
    local keypad
    ...
    TriggerServerEvent('ev-hasino:lvKeypadInput', keypad, prompt.code)
end)

RegisterNetEvent('ev-hasino:keypadSounds', function()
...
RegisterNetEvent('ev-hasino:keypadFailure', function()
    PlaySoundFrontend(-1, 'Hack_Failed', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS', true)
end)

RegisterNetEvent('ev-hasino:keypadSuccess', function()
    PlaySoundFrontend(-1, 'Hack_Success', 'DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS', true)
end)

AddEventHandler('ev-hasino:accessLowerComputer', function()
    ...
    local prompt,abc = RPC.execute('ev-hasino:getCurrentLockboxPrompt')
    exports['ev-ui']:openApplication('hasino-lower-pc', {
        prompts = prompt,
        abc = abc,
    })
end)

AddEventHandler('ev-hasino:takeHeistBag', function(pArgs, pEntity, pContext)
    if DoesEntityExist(pEntity) and pContext.model == drillBagHash then
        TriggerServerEvent('ev-hasino:takeHeistBag', NetworkGetNetworkIdFromEntity(pEntity))
    end
end)

RegisterNetEvent('ev-hasino:drillBox', function(coords, totalRewards, pantherGiven)
    ...
    local bagItem = exports["ev-inventory"]:getItemsOfType("heistduffelbag", 1, true)
    ...
    local moneyBagHash = GetHashKey("ch_prop_ch_moneybag_01a")
    ...
    local drillBag = exports['ev-inventory']:GetAttachedBag()
    if drillBag == 0 then
        deleteBag = true
    ...
    TriggerServerEvent('ev-hasino:robbedLockBox', NetworkGetNetworkIdFromEntity(lockbox), totalRewards, bagInvId)
...
RegisterNetEvent('ev-hasino:beginLowerVault', function()
    enterHasinoVault()
end)
