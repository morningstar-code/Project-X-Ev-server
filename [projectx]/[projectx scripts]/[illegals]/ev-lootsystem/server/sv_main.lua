    ...
    if loot and player then
        for i = 1, pDraws, 1 do
            ...
            if success then
                ...
                TriggerEvent("ev-log:server:CreateLog", "Loot", "Loot", "red", "**" .. GetPlayerName(pServerId) .. "** ( ID: " .. pServerId .. " | State ID: " .. char.id .. ") " .. " Got: " ..item .. " Amount: " .. obtained .. " from loot system")
            end
        end
...
AddEventHandler("loot:useItem", function(itemId)
    ...
    if itemId == 'randomtoy' then
        TriggerEvent("ev:loot:system:draw", src, 'event_randomtoy', 1)
    end
    if itemId == 'randomtoy2' then
        TriggerEvent("ev:loot:system:draw", src, 'event_randomtoy', 1)
    end
    if itemId == 'randomtoy3' then
        TriggerEvent("ev:loot:system:draw", src, 'event_randomtoy', 1)
    end
    if itemId == 'cockegg' then
        TriggerEvent("ev:loot:system:draw", src, 'event_randomtoy', 1)
    end
    if itemId == 'uwutoy' then
        TriggerEvent("ev:loot:system:draw", src, 'event_randomtoy', 1)
    end
end)