...
Citizen.CreateThread(function()
    exports['ev-interact']:AddPeekEntryByModel({-2008643115}, {
        {
            event = 'ev-heists:grid:checkNodeState',
            id = 'checkNodeState',
        ...
        isEnabled = function(pEntity)
            ...
            return active and node and node.state ~= 3 and
              exports['ev-inventory']:hasEnoughOfItem('relayprobe', 1, false, true)
        end,
    })
    
    exports['ev-interact']:AddPeekEntryByModel({-2008643115}, {
        {
            event = 'ev-heists:grid:useThermiteCharge',
            id = 'GridUseThermiteCharge',
        ...
        isEnabled = function(pEntity)
            ...
            return active and node and node.state ~= 3 and (not node.disabled or flashState) and
              exports['ev-inventory']:hasEnoughOfItem('thermitecharge', 1, false, true)
        end,
    })
    
    exports['ev-polytarget']:AddBoxZone('grid_bank_node', vector3(-355.92, -50.55, 54.42), 1.6, 1, {
        heading = 340,
        minZ = 53.42,
        maxZ = 54.82,
    ...
    exports['ev-polytarget']:AddBoxZone('grid_bank_node', vector3(1158.43, 2708.93, 37.97), 2.0, 1, {
        heading = 0,
        minZ = 36.97,
        maxZ = 38.77,
    ...
    exports['ev-polytarget']:AddBoxZone('grid_bank_node', vector3(-2948.24, 481.2, 15.26), 1.8, 1, {
        heading = 355,
        minZ = 14.26,
        maxZ = 16.86,
    ...
    exports['ev-polytarget']:AddBoxZone('grid_bank_node', vector3(-1217.42, -333.12, 42.12), 1.0, 1.6,
        {
            heading = 295,
            minZ = 41.12,
            maxZ = 42.92,
    ...
    exports['ev-polytarget']:AddBoxZone('grid_bank_node', vector3(319.59, -315.69, 51.21), 1.8, 1, {
        heading = 340,
        minZ = 50.21,
        maxZ = 51.61,
    ...
    exports['ev-polytarget']:AddBoxZone('grid_bank_node', vector3(138.96, -1055.9, 29.19), 1.8, 1, {
        heading = 250,
        minZ = 28.19,
        maxZ = 29.59,
    ...
    exports['ev-interact']:AddPeekEntryByPolyTarget('grid_bank_node', {
        {
            event = 'ev-heists:grid:checkBankNodeState',
            id = 'checkBankNodeState',
            icon = 'fas fa-info-circle',
            label = 'check state',
            parameters = {},
        },
...
function startPowerGrid()
    ...
    local nodeData = RPC.execute('ev-heists:grid:getNodeState')
    ...
    local genData = RPC.execute('ev-heists:grid:getGenState')
...
AddEventHandler('ev-heists:grid:checkBankNodeState', function(pArgs, pEntity, pContext)
    mechanicAnim()
    
    local bank = GetNodeContextId(pContext)
    local node = GetBankNode(bank)
    
    local finished = exports['ev-taskbar']:taskBar(1500, 'Getting Info')
    ...
    if finished == 100 then
        local context = {
            ...
            }, 
            {
                title = 'Test Ping', 
                description = "",
                action = 'ev-heists:grid:sendTestPing', 
                key = node
            },
        }
...
AddEventHandler('ev-heists:grid:checkNodeState', function(pArgs, pEntity, pContext)
    mechanicAnim()
    local node = GetNodeForEntity(pEntity)
    if not node then
    ...
    if finished == 100 then
        local context = {
            ...
            },
            {
                ...
                description = "",
                action = 'ev-heists:grid:viewConnected', 
                key = node
            },
        }
        exports['ev-ui']:showContextMenu(context)
...
RegisterUICallback('ev-heists:grid:viewConnected', function(data, cb)
    cb({data = {}, meta = {ok = true, message = 'done'}})
    local node = data.key
    local connected = RPC.execute('ev-heists:grid:getConnected', node.id)
...
RegisterUICallback('ev-heists:grid:sendTestPing', function(data, cb)
    cb({data = {}, meta = {ok = true, message = 'done'}})
    local node = data.key
    
    local success = RPC.execute('ev-heists:grid:pingNode', node)
    if not success then
...
AddEventHandler('ev-heists:grid:useThermiteCharge', function(pArgs, pEntity, pContext)
    local node = GetNodeForEntity(pEntity)
    if not node then
    ...
    sendAlert()
    if success then
        RPC.execute('ev-heists:grid:useThermiteCharge', node)
    end
end)

AddEventHandler('ev-inventory:itemUsed', function(pItemId, pInfo)
    ...
    if pItemId == 'heistlaptop333' then -- disabled
        if not exports['ev-inventory']:hasEnoughOfItem('heistusb5', 1, false, true) then
        ...
        Citizen.CreateThread(function()
            ...
            if MinigameResult then
                ...
                startPowerGrid()
                TriggerServerEvent('ev-heists:hack:success', minigameHackExp)
            end
            MinigameResult = nil
        end)
        return
...
RegisterNetEvent('ev-heists:grid:setActive', function(pState)
    active = pState
    if not active then
        for _, node in ipairs(Nodes) do
    ...
    exports['ev-island']:hideBlips(active)
end)

RegisterNetEvent('ev-heists:grid:updateNodeState', function(pNode)
    for idx, node in ipairs(Nodes) do
...
RegisterNetEvent('ev-heists:grid:updateGenState', function(pGenId, pState)
    for _, gen in ipairs(Generators) do
...
-- RegisterCommand('grid', function(source, args, RawCommand)
--     startPowerGrid()
-- end)