...
RegisterNetEvent("ev-heists:updateDoorStatus")
AddEventHandler("ev-heists:updateDoorStatus", function(name, heading, frameCount)
    prevHeadings[name] = heading
    if doorEntities[name] == nil then return end
    ChangeDoorHeading(doorEntities[name], heading, frameCount)
end)

AddEventHandler("ev-heists:doors:vaultDoor", function(pParams)
