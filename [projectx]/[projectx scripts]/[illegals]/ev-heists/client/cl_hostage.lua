...
local function release(entity)
    grabbed = false
    exports["ev-ui"]:hideInteraction("success")
...
local function grab(entity)
    ...
    Wait(2000)
    exports["ev-ui"]:showInteraction("[F] Release!", "success")
...
local function showInteraction()
    if not interactionUp then
        ...
        exports["ev-ui"]:showInteraction("[G] Grab!", "error")
    end
end
local function hideInteraction()
    if interactionUp then
        listening = false
        interactionUp = false
        exports["ev-ui"]:hideInteraction("error")
    end
end
...
AddEventHandler("ev-casino:casinoEnteredEvent", function() inCasino = true end)
AddEventHandler("ev-casino:casinoExitedEvent", function() inCasino = false end)

AddEventHandler('ev-heist:disableHostage', function(pDisabled)
    ...
    while true do
        ...
        then
            ...
            if lastEntity ~= entity then
                local flags = exports["ev-flags"]:GetPedFlags(entity)
