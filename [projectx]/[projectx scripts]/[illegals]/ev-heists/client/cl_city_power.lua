...
AddEventHandler("ev-inventory:itemUsed", function(item)
    ...
    exports["ev-dispatch"]:dispatch("10-37", "PowerPlant Atack") 
    ...
    RPC.execute("ev-heists:cityPowerDoorOpen", activeEntranceDoor)
end)

RegisterNetEvent('ev-heists:checkThings')
AddEventHandler('ev-heists:checkThings', function()
    Citizen.CreateThread(function()
        while inPlant do
            Citizen.Wait(1)
            local explosion = IsExplosionInRadius(powerPanelCoords.x, powerPanelCoords.y, powerPanelCoords.z, 10)
            if explosion then
                RPC.execute('ev-heists:cityPowerExplosion')
                break
            end
        end
    end)
...
RegisterNetEvent('ev-heists:addExplosion')
AddEventHandler('ev-heists:addExplosion', function(coords)
...
AddEventHandler("ev-polyzone:enter", function(pZone)
    ...
    inPlant = true
    TriggerEvent('ev-heists:checkThings')
end)
AddEventHandler("ev-polyzone:exit", function(pZone)
    if pZone ~= "powerplant" then return end
    inPlant = false
...
Citizen.CreateThread(function()
  -- hospital
  exports["ev-polyzone"]:AddBoxZone("power_generator", vector3(333.23, -585.39, 43.28), 45.4, 62.8, {
    heading=339,
    minZ=41.88,
    maxZ=45.68,
  })
  -- mrpd
  exports["ev-polyzone"]:AddBoxZone("power_generator", vector3(455.67, -992.97, 30.68), 50.0, 65.0, {
    heading=0,
    minZ=16.68,
    maxZ=43.88,
  })

  exports["ev-polyzone"]:AddBoxZone("powerplant", vector3(717.39, 139.71, 80.75), 59.6, 71.0, {
    name="powerplant",
    heading=340,
  })
  ...
  -- court house
  exports["ev-polyzone"]:AddPolyZone("power_generator", {
