...
function VaultLowerUsePanel(laptopId)
    ...
    if not success then
        RPC.execute("ev-heists:vaultLowerPanelFail")
        return
    end
...
RegisterNetEvent("ev-heists:swapLowerVaultIPL")
AddEventHandler("ev-heists:swapLowerVaultIPL", function(state, uState)
...
Citizen.CreateThread(function()
    local result = RPC.execute("ev-heists:getVaultLowerState")
...
AddEventHandler("ev-polyzone:enter", function(name)
    ...
    RPC.execute("ev-heists:lowerVaultEntranceEnter")
end)

AddEventHandler("heists:vaultLowerTrolleyGrab", function(loc, type)
    local canGrab = RPC.execute("ev-heists:vaultLowerCanGrabTrolley", loc, type)
    if canGrab then
        Loot(type)
...
Citizen.CreateThread(function()
  exports["ev-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(286.53, 220.17, 97.69), 0.4, 0.4, {
    heading=0,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(284.7, 221.63, 97.69), 0.4, 0.4, {
    heading=340,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(286.83, 227.45, 97.69), 0.4, 0.4, {
    heading=340,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports["ev-polytarget"]:AddBoxZone("lower_vault_keypad", vector3(289.21, 227.46, 97.69), 0.4, 0.4, {
    heading=325,
    minZ=98.09,
    maxZ=98.49,
    data = {
  ...
  exports['ev-interact']:AddPeekEntryByPolyTarget('lower_vault_keypad', {{
    event = "ev-heists:lowerVaultPanelPush",
...
local publicGameFinishedEndpointNumbers = 'ev-ui:heistsLVPublicMinigameNumbersEndpoint'
local activePanelId = 0
RegisterUICallback(publicGameFinishedEndpointNumbers, function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  RPC.execute("ev-heists:lowerVaultPanelPublicPush", activePanelId, data.success)
  activePanelId = 0
end)
AddEventHandler("ev-heists:lowerVaultPanelPush", function(p1, p2, pArgs)
  local id = pArgs.zones["lower_vault_keypad"].id
  exports['ev-ui']:openApplication('textbox', {
    callbackUrl = 'ev-ui:heists:lowerVaultPanel',
    inputKey = id,
    items = {
      {
        icon = "user-secret",
...
RegisterUICallback("ev-ui:heists:lowerVaultPanel", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports['ev-ui']:closeApplication('textbox')
  local currentKeypadId = data.inputKey
...
--
local gameFinishedEndpointNumbers = 'ev-ui:heistsLVMinigameNumbersEndpoint'
AddEventHandler("ev-heists:vaultLowerKeyboardInteract", function()
  local hasUsb = exports['ev-inventory']:hasEnoughOfItem('vcomputerusb', 1, false, true)
  if not hasUsb then
    TriggerEvent("DoLongHudText", "Missing PC USB stick", 2)
    return
  end
  RPC.execute("ev-heists:registerLVUsbUse")
  exports["ev-ui"]:openApplication("minigame-numbers", {
