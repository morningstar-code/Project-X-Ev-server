...
MinigameResult = nil
MinigameUICallbackUrl = "ev-ui:heistsPanelMinigameResult"
...
function UseBankPanel(panelCoords, panelHeading, location, withholdEmail)
    ...
    -- if not isDarkMarketBanks then 
    --   return ClearPedTasksImmediately(ply)
    -- end

    exports["ev-hacking"]:OpenHackingGame(function(success)
      if success then 
        MinigameResult = success
        RPC.execute("completeTask", "darkmarket_banks", 2)
        RPC.execute("ev:progression:character:illegalRep", minigameHackExp, "hackexp")
      else
        MinigameResult = success
        RPC.execute("completeTask", "darkmarket_banks", 3, nil, true)
      end
    end, gameRoundsTotal, numberOfShapes, gameDuration)
    ...
    Citizen.CreateThread(function()
        ...
        if MinigameResult and not withholdEmail then
          Citizen.CreateThread(function()
            Citizen.Wait(2500)
            TriggerEvent('phone:emailReceived', 'Dark Market', '#A-1001', 'Nice! You bypassed the captcha. Give me a few moments to open the door!')
          end)
        end
        if MinigameResult then
          TriggerServerEvent("ev-heists:hack:success", minigameHackExp)
        end
        TriggerServerEvent("ev-heists:hack:track", MinigameResult, "laptop")
...
function LoopSkill(count)
    ...
    while loopCount < count do
        ...
        local finished = exports["ev-ui"]:taskBarSkill(math.random(1000, 4000), math.random(7, 12))
  ...
  Citizen.CreateThread(function()
    ...
    if gameSettings ~= nil then
      exports["ev-memory"]:OpenThermiteGame(function(success)
        if success then
          thermiteMinigameResult = success
            RPC.execute("completeTask", "darkmarket_stores", 2)
        else
          thermiteMinigameResult = success
            RPC.execute("completeTask", "darkmarket_stores", 3, nil, true)
        end
    end, gameSettings.gridSize, gameSettings.coloredSquares, gameSettings.gameTimeoutDuration * thermiteTimeMultiplier, gameSettings.gameFinishedEndpoint)
    ...
    -- else
    --   exports["ev-ui"]:openApplication("memorygame", {
    ...
    TriggerServerEvent("ev-heists:hack:track", thermiteMinigameResult, "thermite")
...
local minigamePracUICallbackUrl = "ev-ui:heistsPanelMinigameResultPrac"
...
AddEventHandler("ev-inventory:itemUsed", function(item)
  ...
  if minigamePracResult then
    TriggerServerEvent("ev-heists:hack:success", 1)
  end
end)

AddEventHandler("ev-heists:purchasePracticeLaptop", function()
  RPC.execute("ev-heists:purchasePracticeLaptop")
end)


RegisterNetEvent("startillegalbanks")
AddEventHandler("startillegalbanks", function(groupId, groupData, members, task, pState)
...
RegisterNetEvent("finishillegalbanks")
AddEventHandler("finishillegalbanks", function(members, pState)
  local clientId = PlayerId()
  local src = GetPlayerServerId(clientId)
  for k,v in pairs(members) do
      if tonumber(v.src) == tonumber(src) then
