-- -930.40881347656, 310.7903137207, 71.082565307617 Heading: 29.9499168396
Config = {
  ...
  startped = {
    [1] = vector4(-1128.69, 2695.956, 17.800, 81.61)
  },
  ...
  Corners = {
    ...
    [7] = vector3(-930.40881, 310.79031, 71.08256),
  ...
  CarStopLoc = {
    [1] = vector3(1577.2073, 3706.8645, 34.499),
    [2] = vector3(-156.1994, -2032.6090, 22.736),
    [3] = vector3(675.3199, 206.9351, 93.578),
    [4] = vector3(-361.0020, -191.6637, 37.4415),
    [5] = vector3(155.6842, -1742.0272, 29.0844),
    [6] = vector3(-1326.974, -1049.346, 7.4170861),
    [7] = vector3(-930.40881, 310.79031, 71.08256),
    [8] = vector3(-24.5940, 6514.2138, 31.3952),
    [9] = vector3(1789.41503, 4574.8769, 37.09510),
    [10] = vector3(1229.48034, -1690.56030, 39.76789)
  },
...
RegisterNetEvent('start-oxy', function()
  if exports['ev-inventory']:hasEnoughOfItem('darkmarketdeliveries', 1) and exports['ev-inventory']:hasEnoughOfItem('vpnxj', 1) then
    local info = exports["ev-inventory"]:GetInfoForFirstItemOfName("darkmarketdeliveries")
    if info.quality <= 0 then
      TriggerEvent('DoLongHudText', 'Seems your delivery list is ruined, You need a new one!', 2)
      TriggerEvent("inventory:removeItem", "darkmarketdeliveries", 1)
      return
    end

    TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', 'Wait for a job offer!')
    Citizen.Wait(math.random(60000, 120000))

    local accepted = exports['ev-phone']:DoPhoneConfirmation(30, "Dark Market", 'I found you a run, You ready?', "horse-head");
    if (not accepted) then
      Citizen.Wait(1000)
      TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', 'You declined the offer!')
      return
    end
...
AddEventHandler('ev-oxy:Accept:Offer', function()
  TriggerServerEvent('started-job_oxy')
  TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', 'Head towards the location on your GPS')
end)
...
RegisterNetEvent('collect-package', function()
  if toplampaket < 10 then
    ...
    if not exports['ev-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
      ...
      TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', '(' .. toplampaket .. '/10) Packages Collected.')

      if toplampaket == 10 then
        TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', 'All Packages Collected.')
...
RegisterNetEvent('generate-new-corner-loc', function()
  ...
  jobs = jobs + 1
  TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', 'Drive to the next location.')
...
RegisterNetEvent('spawn-car-client', function()
  ...
  TaskVehicleDriveToCoord(ped, veh, Config.CarStopLoc[oldugum_bolge].x, Config.CarStopLoc[oldugum_bolge].y, Config.CarStopLoc[oldugum_bolge].z, 10.0, false, model, 316,
    0.1, false)
  ...
  if exports['ev-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
    if licensePlate == pPlate then
      ...
      if handoff < 5 then
        ...
        handoff = handoff + 1
        TriggerEvent('phone:emailReceived', 'Dark Market', 'Anonymous', '(' .. handoff .. '/5) Packages Handed Off.')
        ...
        if exports['ev-inventory']:hasEnoughOfItem('darkmarketpackage', 1, true) then
          ...
          if rnd == 1 then
            ...
            if math <= 2 then
              ...
              TriggerEvent("player:receiveItem", "oxy", math + 1)
            end
            --if you have a cashroll in your inventory, he will give you $100 back for it.
            --TODO: Fix me, I'm exploitable, like right now please.
            if exports["ev-inventory"]:hasEnoughOfItem("cashroll", 1) then
              TriggerEvent("inventory:removeItem", "cashroll", 1)
              TriggerServerEvent("ev-activities:oxy") 
            end
          end
...
AddEventHandler("ev-oxyruns:clean", function(pRandomChance)
  ...
  for typ, conf in pairs(billsCleaningStuff) do
    ...
    if math.random() < randomChance and exports["ev-inventory"]:hasEnoughOfItem(typ, totalAmount, false) then
      ...
      local buff = exports['ev-buffs']:shouldIncreaseJobPayout()
      if buff then
...
function thread()
  ...
  if Config.TargetName == 'q' then
    exports["ev-polytarget"]:AddCircleZone("oxystart", vector3(-1128.69, 2695.956, 17.800), 0.70, {
      useZ = true,
    })
