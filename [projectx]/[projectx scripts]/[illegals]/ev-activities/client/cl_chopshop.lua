...
AddEventHandler("ev-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == aspectChopCar then
        ...
        aspectStartChopping()
        exports['ev-ui']:showInteraction('[E] Start Chopping')
    end
end)
...
function RhodoDisptachCunt()
	local math = math.random(1,99)
    if math <= 25 then
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    if success then
        TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'I will look for another job for you keep an eye on your phone!')
        DeleteEntity(vehicle)
        FreezeEntityPosition(PlayerPedId(),false)
    end

    TriggerEvent('ev-chop:cooldown')
    return success
end

RegisterNetEvent('ev-chop:cooldown')
AddEventHandler('ev-chop:cooldown', function()
    CoolDownActive = true
    Citizen.Wait(math.random(60000, 120000)) 
    CoolDownActive = false
    if ClockedIntoChop then 
        local accepted = exports['ev-phone']:DoPhoneConfirmation( 30, "Chop Shop", 'I found a job for you, You ready?', "horse-head");
        if (not accepted) then
            Citizen.Wait(1000)
            TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'You declined the offer!')
            return
        end
        TriggerEvent('ev-chopshop:acceptOffer')
    end
end)
                ...
                if bone.type == "door" and distance <= 1.6 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if CoolDownActive then
                            TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'You recently chopped come back later.')
                        else
...
AddEventHandler('ev-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'You have a active cooldown! Please wait.')
    elseif ClockedIntoChop then
        TriggerEvent('ev-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'Wait for a job offer!') 
        Citizen.Wait(math.random(60000, 120000)) 
...
AddEventHandler('ev-chopshop:clock-out', function()
    if ClockedIntoChop then
        ...
        TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'Clocked Out!')
...
function CreatePickupCarBlip(x, y ,z)
  ...
  RegisterNetEvent('ev-chopshop:acceptOffer')
  AddEventHandler('ev-chopshop:acceptOffer', function()
    CreateChopCar()
  end)
...
function CreateChopCar()
    ...
    TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'Find the vehicle to chop')
    ...
    Citizen.CreateThread(function()
        while true do
          ...
          if GetVehiclePedIsIn(PlayerPedId(), false) == aspectChopCar then
                ...
                TriggerEvent('phone:emailReceived', 'Chop Shop', 'Anonymous', 'Drive to the GPS and begin Chopping') 
                RemoveBlip(blipPickup)
            return
          end
        end
