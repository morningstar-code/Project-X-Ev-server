...
Citizen.CreateThread(function()
  ...
  exports['ev-interact']:AddPeekEntryByModel(pedModelKeyMap, {
    {
      event = 'ev-heists-serverroom:lootDeadGuard',
      id = 'ev-heists-serverroom:lootDeadGuard',
      icon = 'circle',
      label = 'Grab Gear',
      parameters = {},
    },
...
local function controlPed(model, pedNetId, aimingPed, pedString)
  Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
      RPC.execute("ev-heists-serverroom:registerPedId", pedNetId)
    end)
  ...
  Citizen.CreateThread(function()
    while inServerFarm do
      if isHeistActive and canSpawnAnotherPed() then
        ...
        local pedNetId, pedString = RPC.execute("ev-heists:npcs:spawn", {
          type = "server_farm",
          npc = {4, model.model},
          spawnCoords = id,
...
function teleportToServerFarm(pComputerCode)
  local hasHelmet = exports['ev-inventory']:hasEnoughOfItem("varhelmet", 1, false, true)
  if not hasHelmet then
    TriggerEvent("DoLongHudText", "You need a VAR helmet!", 2)
    return
  end
  local cid = exports["isPed"]:isPed("cid")
  local isEntryTimeoutEnabled = RPC.execute("ev-heists-serverroom:isEntryTimeoutEnabled")
  if isEntryTimeoutEnabled then
    TriggerEvent("DoLongHudText", "System is recovering, please check in a few minutes...", 2)
    return
  end
  local isHeistActiveCheck = RPC.execute("ev-heists-serverroom:getHeistStatus", cid)
  if isHeistActiveCheck then
    TriggerEvent("DoLongHudText", "Cannot enter right now", 2)
    return
  end
  local isTerminalActiveCheck = RPC.execute("ev-heists-serverroom:getHeistTerminalStatus", pComputerCode, cid)
  ...
  StartTracking()
  allPeds = {}
  TriggerEvent("ev-menu:var:inServerFarm", true)
  DoScreenFadeOut(400)
  Wait(600)
...
RegisterNetEvent("ev-heists-serverroom:activateHeist", function(pIsActive, pHeistSpawnMultiplier, pComputersHacked, pBlackoutEnabled)
  ...
  if inServerFarm and (not isHeistActive) then
    ...
    Wait(2000)
    TriggerEvent("ev-heists-serverroom:vr-room-exit")
  end
end)

RegisterNetEvent("ev-heists-serverroom:deletePeds", function()
  deletePeds()
end)

RegisterNetEvent("ev-heists-serverroom:heistSpawnMultiplier", function(pHeistSpawnMultiplier, pComputersSolved, pBlackoutEnabled)
  heistSpawnMultiplier = pHeistSpawnMultiplier
...
RegisterNetEvent("ev-heists-serverroom:maxPedCountSpawn", function(pMaxPedCount)
  MAX_PED_SPAWN_AMOUNT = pMaxPedCount
end)

AddEventHandler("ev-heists-serverroom:vr-room-enter", function(p1, p2, pContext)
  local computerCode = pContext.zones["vr_room_entrance"].id
  teleportToServerFarm(computerCode)
end)

AddEventHandler("ev-heists-serverroom:vr-room-exit", function()
  if not inServerFarm then return end
  DoScreenFadeOut(400)
  deletePeds()
  ...
  inServerFarm = false
  EndTracking()
  TriggerEvent("ev-menu:var:inServerFarm", false)
  RPC.execute("ev-heists-serverroom:playerExitVar", activeComputerCode)
  Wait(600)
...
RegisterUICallback("ev-ui:heists:serverroomComputerTextbox", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "" } })
  exports["ev-ui"]:closeApplication("textbox")
  local computerId = data.inputKey.computerId
  local successfulInput = RPC.execute("ev-heists-serverroom:checkComputerInput", computerId, data.values.input)
  if not successfulInput then
    TriggerEvent("DoLongHudText", "Invalid input.")
    return
  end
  RPC.execute("ev-heists-serverroom:updateComputerStatus", computerId, 2)
  TriggerEvent("DoLongHudText", "Input accepted.")
end)

RegisterUICallback("ev-ui:minigame:movingNumbers", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "" } })
...
AddEventHandler("ev-heists-serverroom:hackUpstairsComputer", function(p1, p2, pData)
  local computerId = pData.zones["server_farm_computer"].id
  local computerStatus = RPC.execute("ev-heists-serverroom:getComputerStatus", computerId)
  ...
  if computerStatus == 0 then
    ...
    if not COMPUTER_HACK_SUCCESS then
      RPC.execute("ev-heists-serverroom:registerComputerFailure")
      return endComputerInteract()
    end
    local hubLocation = RPC.execute("ev-heists-serverroom:getHubLocation", computerId)
    if DEBUG_MODE then
    ...
    RPC.execute("ev-heists-serverroom:updateComputerStatus", computerId, 1)
    return endComputerInteract()
  end
  if computerStatus == 1 then
    exports["ev-ui"]:openApplication("textbox", {
      callbackUrl = "ev-ui:heists:serverroomComputerTextbox",
      inputKey = { computerId = computerId },
      items = {
        {
          type = "password",
          icon = "pencil-alt",
...
AddEventHandler("ev-heists-serverroom:hackDownstairsComputer", function(p1, p2, pData)
  local computerId = pData.zones["server_farm_computer_down"].id
  local squares = RPC.execute("ev-heists-serverroom:getComputerSquares", computerId)
  TriggerEvent("animation:PlayAnimation", "type")
  COMPUTER_HACK_SUCCESS = nil
  exports["ev-ui"]:openApplication("minigame-serverroom", {
    numberTimeout = 5000,
  ...
  local finished = exports["ev-taskbar"]:taskBar(5000, "Downloading Encryption Data")
  PlaySoundFrontend(-1, "download_complete", "DLC_BTL_Break_In_Sounds", true)
  ClearPedTasks(PlayerPedId())
  RPC.execute("ev-heists-serverroom:getEncryptionUsb", computerId)
end)

local lootingDeadGuard = false
AddEventHandler("ev-heists-serverroom:lootDeadGuard", function(p1, pEntity, p3)
  if lootingDeadGuard then return end
  lootingDeadGuard = true
  TriggerEvent("farm:weed")
  RPC.execute("ev-heists-serveroom:lootDeadGuard")
  Wait(3000)
...
AddEventHandler("ev-heists-serverroom:entryBoxInteract", function(p1, p2, p3)
  ...
  local finished = exports["ev-taskbar"]:taskBar(DEBUG_MODE and 1000 or 20000, "Configuring Server Panel")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then
    return
  end
  local computerCode = RPC.execute("ev-heists-serverroom:getComputerCodeFromHubBox", data)
  if DEBUG_MODE then
...
AddEventHandler("ev-heists:serverroom:revivePlayer", function(pEntity)
...
AddEventHandler("ev-inventory:itemUsed", function(pItem, pInfo)
  ...
  for k, _ in pairs(sources) do
    local items = exports["ev-inventory"]:GetItemsByItemMetaKVMulti("heistusbsr", {
      encryption_id = id,
      encryption_source = k,
    })
    if #items > 0 then
...
AddEventHandler("ev-heists-serverroom:entryBoxDecrypt", function(p1, p2, p3)
  if not p3 or not p3.zones or not p3.zones["server_farm_hub_box"] then return end
  local data = p3.zones["server_farm_hub_box"]
  local info = exports["ev-inventory"]:GetInfoForFirstItemOfName("heistusbsrmk")
  ...
  TaskPlayAnim(PlayerPedId(), animDict, "look_around_v5_monitor", 8.0, -8.0, -1, 1, 1.0, false, false, false)
  local finished = exports["ev-taskbar"]:taskBar(DEBUG_MODE and 1000 or 2500, "Decrypting Server Panel")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end
  RPC.execute("ev-heists-serverroom:decryptServerBox", id, data.code .. "-" .. data.key)
end)
