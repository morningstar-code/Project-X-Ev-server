...
function createFarmPolys()
  ...
  for _, settings in pairs(COORDS_FOR_SERVER_BOXES) do
    ...
    for k, z in pairs(((settings.label == "xm_base_cia_serverhub_01") and zones1 or zones2)) do
      ...
      -- TriggerServerEvent("ev-scenes:addSceneToClients", {
      --   coords = coords,
  ...
  for _, z in pairs(newZones) do
    exports["ev-polytarget"]:AddBoxZone("server_farm_hub_box", z[1], z[2], z[3], z[4])
  end

  exports["ev-interact"]:AddPeekEntryByPolyTarget("server_farm_hub_box", {{
    event = "ev-heists-serverroom:entryBoxInteract",
  ...
  exports["ev-interact"]:AddPeekEntryByPolyTarget("server_farm_hub_box", {{
    event = "ev-heists-serverroom:entryBoxDecrypt",
    ...
    distance = { radius = 2.0 },
    isEnabled = function()
      local hasItem = exports["ev-inventory"]:hasEnoughOfItem("heistusbsrmk", 1, false, true)
      return hasItem
    end,
  })
end
