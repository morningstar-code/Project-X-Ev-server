...
AddEventHandler("ev-island:enableIsland", function(pEnabled)
  if pEnabled then
    ...
    SetWindSpeed(0.0)
    exports['ev-weather']:FreezeWeather(true, 'Very Sunny')
    SetToggleMinimapHeistIsland(true)
    TriggerEvent('ev-island:onIsland', true)
    TriggerEvent('ev-island:hideBlips', true)
    Wait(0)
  ...
  TriggerEvent('ev-island:onIsland', false)
  TriggerEvent('ev-island:hideBlips', false)
  exports['ev-weather']:FreezeWeather(false, 'Very Sunny')
end)

RegisterNetEvent("ev-island:enableSwapping", function(pEnabled)
  isIslandSwappingEnabled = pEnabled
end)
Citizen.CreateThread(function()
  Citizen.Wait(5000)
  TriggerServerEvent("ev-island:checkIslandSwapping")
end)

local state = false
if GetConvar("sv_environment", "prod") == "debug" then
  RegisterCommand('island', function(source, args, RawCommand)
    state = not state
    TriggerEvent("ev-island:enableIsland", state)
  end)
end

exports('hideBlips', function(pState)
  TriggerEvent('ev-island:hideBlips', pState)
end)
