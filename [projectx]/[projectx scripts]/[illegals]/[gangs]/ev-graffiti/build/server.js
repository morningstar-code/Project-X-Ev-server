;(() => {
    ...
    onNet("ev-graffiti:completePurchase", async (pId, isSharlock) => {
        let src = source
        let data = global.exports["ev-lib"].getCharacter(src)

        if (isSharlock) {
            let currMode = info[pId]
            let metadata = {
                name : currMode.name,
                model: currMode.model,
                isGang: currMode.isGang,
                gangId: pId,
                _image_url : currMode.image,
                _hideKeys: ["model", "gangId", "_image_url", "isGang"]
            }
            emitNet('player:receiveItem', src, "spraycan", 1, false, [], metadata)
        } else {
            if (data.cash > 5000) {
                let currMode = info[pId]
                let metadata = {
                ...
                emitNet('DoLongHudText', src, 'Not enoguh money', 2)
            }
        }
    })

    onNet("ev-graffiti:changeSpray", async (data, data2) => {
        console.log(data, data2)

    })

    onNet("ev-gangsystem:contestGraffiti", async (data, data2, data3) => {
    ...
    onNet('ev-objects:requestObjects', async () => {
        let src = source,
          Graffiti = await SQL.execute('SELECT * FROM graffiti', {}),
          requestGraffiti = []
        for (let index = 0; index < Graffiti.length; index++) {
          let pCoords = JSON.parse(Graffiti[Number(index)].coordinates)
          let pRotation = JSON.parse(Graffiti[Number(index)].rotation)
          let coords = {
              x: pCoords.x,
              y: pCoords.y,
              z: pCoords.z,
        ...
        let rotations = {
            ...
            requestGraffiti.push({
            id: Graffiti[Number(index)].id,
            model: Graffiti[Number(index)].model,
            coordinates: coords,
            rotation: rotations,
            metaData: JSON.parse(Graffiti[Number(index)].metaData),
            group: "graffiti",
            ns : "graffiti",
          })
        }
        emitNet('ev-objects:loadObjects', src, requestGraffiti)
      })

    _0x253f2c.register("ev-graffiti:spray", async (src, pModel, pCoords, pRotation, pGandId, pIsGang) => {
        let currMode = info[pGandId].model
        let data = {
            ns: "graffiti",
            gangId: pGandId,
            isGang: pIsGang
        ...
        let coords = {
          ...
          ObjId =
            ...
            Math.floor(10 * Math.random()).toString()
        if (
          !(await SQL.execute('INSERT INTO graffiti (model, coordinates, rotation, metaData, randomId) VALUES (@model, @coordinates, @rotation, @metaData, @randomId)',
            {
              model: currMode,
              coordinates: JSON.stringify(coords),
              rotation: JSON.stringify(pRotation),
        ...
        let Graffiti = await SQL.execute('SELECT * FROM graffiti WHERE randomId = @randomId',
        { randomId: ObjId }
      )
      if (!Graffiti[0]) {
        return false
      ...
      let CurrGraffiti = {
        ...
        return emitNet('ev:objects:prepareNewObject', -1, CurrGraffiti), true
    })
})()
