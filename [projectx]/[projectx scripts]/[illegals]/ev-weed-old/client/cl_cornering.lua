...
local RequireScales = true
local DropEvidence = true
...
AddEventHandler('ev-weed:startCornering', function(pContext, pEntity)
  ...
  -- Ped spawn loop
  Citizen.CreateThread(function()
    while CurrentlyCornering and 45000 ~= -1 do
      PopulateNow()
      Wait(45000)
    end
  end)
...
AddEventHandler('ev-weed:cornerSale', function(pContext, pEntity)
  ...
  if DropEvidence and math.random() < 0.3 then
...
AddEventHandler('ev-weed:prepareBaggies', function(pContext, pEntity)
  ...
  local hasBaggies = exports['ev-inventory']:hasEnoughOfItem('emptybaggies', 7, false)
  if not hasBaggies then
