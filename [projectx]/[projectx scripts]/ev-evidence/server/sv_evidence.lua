...
RPC.register("ev-evidence:getDNA", function(src)
...
RPC.register("ev-evidence:fetchEvidence", function(src)
    return true, DroppedEvidences
end)

RPC.register("ev-evidence:addEvidence", function(src, pDropped)
...
RPC.register("ev-evidence:clearEvidence", function(src, pCoords, pEvidence)
    ...
    for i, v in ipairs(players) do
        ...
        if #(pCoords - coords) < 50.0 then
            TriggerClientEvent("ev-evidence:clearCache", v)
        end
    end
