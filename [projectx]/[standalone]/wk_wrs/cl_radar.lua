...
AddEventHandler("platecheck:frontradar", function()
    TriggerServerEvent('ev:vehicles:plateCheck',radarInfo.frontPlate)
end)

RegisterNetEvent("platecheck:rearradar")
AddEventHandler("platecheck:rearradar", function()
    TriggerServerEvent('ev:vehicles:plateCheck',radarInfo.rearPlate)
end)
...
function isVehicleFlagged(veh, plate)
    return exports["ev-vehicles"]:GetVehicleMetadata(veh, "isFlagged") --TODO: Make this work via MDT or something 
    --RPC.execute('ev:vehicles:isFlagged', plate) 
end
...
Citizen.CreateThread( function()
    ...
    while true do 
        ...
        Citizen.Wait( 100 ) --Lowered to make it more seamless as it was slow, 0.02 when radar is open, 0.00 when off.
    end
end )
