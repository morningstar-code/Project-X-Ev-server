...
toggleFaceWear = function(pWearType, pShouldRemove)
  ...
  if pWearType == 1 and not pShouldRemove and removedHatManually then
    local hasHat = exports["ev-inventory"]:hasEnoughOfItem("hat", 1, false, true, {hat = facialWear[PropIndex+1]["Prop"]})
    if not hasHat then
  ...
  if pWearType == 4 and not pShouldRemove and removedMaskManually then
    local hasMask = exports["ev-inventory"]:hasEnoughOfItem("mask", 1, false, true, {mask = facialWear[pWearType]["Prop"]})
    if not hasMask then
    ...
    if pWearType ~= 5 then
      ...
      if pWearType == 4 then
        ...
        if currentDrawable ~= -1 then
          ...
          local hasPropItem = exports["ev-inventory"]:hasEnoughOfItem("mask", 1, false, true, itemMeta)
        ...
        if pWearType ~= 2 then
          ...
          if pWearType == 1 and currentProp ~= -1 then
            ...
            local hasPropItem = exports["ev-inventory"]:hasEnoughOfItem("hat", 1, false, true, itemMeta)
            if not hasPropItem then
...
local bedOffsets = {
  [337126451] = vector3(-0.3, -0.2, 0.9),
  [1901643221] = vector3(0, 0.1, 1.0),
}
local bedRots = {
  [1901643221] = 90.0,
}
  ...
  if DoesEntityExist(objFound) then
    -- TriggerEvent("DoLongHudText","Press [E] to leave the bed at any time.",1)
      ...
      if not disableKeys then
        showInteraction = true
        exports["ev-ui"]:showInteraction("[E] Leave Bed; [F] Dismiss")
      end

    local bedOffset = vector3(-0.2, 0.1, 1.4)
    if bedOffsets[GetEntityModel(objFound)] ~= nil then
      bedOffset = bedOffsets[GetEntityModel(objFound)]
    end
    if GetEntityModel(objFound) == -289946279 or GetEntityModel(objFound) == -1519439119 then
      ...
      while sleeping do
        local rot = 180.0
        if bedRots[GetEntityModel(objFound)] ~= nil then
          rot = bedRots[GetEntityModel(objFound)]
        end
...
Citizen.CreateThread(function()
  while true do
    local isSitting = exports["ev-flags"]:HasPedFlag(PlayerPedId(), "isSittingOnChair")
...
RegisterNetEvent('ev-voice:focus:set')
AddEventHandler('ev-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
AddEventHandler("police:setClientMeta",function(meta)
  ...
  stresslevel = meta.stress

  TriggerEvent("client:updateStress", stresslevel)
end)

RegisterNetEvent('ev:admin:maxstats') -- ev-admin:maxstats
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if updateCounter == 0 then
      ...
      if valueChanged or shouldUpdate then
        -- TriggerEvent("ev-ui:hud:values", currentValues)
...
Citizen.CreateThread(function()
  while true do
    if citrineStartTime ~= 0 and (not stressIsDead) then
      DepleteFood()
      exports['ragdoll']:SetPlayerHealth(GetEntityHealth(PlayerPedId()) + 4)
...
AddEventHandler("ev-inventory:itemUsed", function(item)
  ...
  local finished = exports["ev-taskbar"]:taskBar(4000, "Using Chapstick")
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  local hasLicense = exports["ev-gov"]:HasLicense("pilot_license", source)
  if hasLicense then
    return
  end
