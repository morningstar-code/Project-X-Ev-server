...
RegisterNetEvent("ev-inventory:itemCheck", function(item, hasItem, quantity)
    ...
    if not hasItem then
        if item == "boxinggloves" then
            ...
            TriggerEvent("ev-inventory:boxingGlovesEquipped", false)
        end
        SetDefaultValues()
        return
    end
    ...
    for buff, value in pairs(buffs) do
        ...
        if buff == "meleedamage" then
            ...
            if item == "boxinggloves" then
                ...
                TriggerEvent("ev-inventory:boxingGlovesEquipped", true)
...
Citizen.CreateThread(function()
    ...
    local buffsEnabled = exports["ev-config"]:GetMiscConfig("buffs.enabled")
