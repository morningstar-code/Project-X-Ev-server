...
local currentAnimTree, currentAnim = nil, nil
...
AddEventHandler('hasSignedOnEms', function()
  TriggerServerEvent("ev:voice:radio:addPlayerToRadio", 2.0, GetPlayerServerId(PlayerId()))
  isEMS = true
end)
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if IsEntityDead(player) then
      if exports['ev-admin']:IsInGodMode() then
        local ped = PlayerPedId()
        TriggerEvent("Heal")
        SetMaxHealth()
        SetMaxArmor()
      ...
      -- local isHardcore = exports['ev-character']:GetCharacterType() == 'hardcore'
      -- print("isHardcore", isHardcore)
      -- if isHardcore then
      --     local score = 40
      --     if isMinor then
      --         score = 10
      ...
      --         score = score * 2
      --     end
      --     exports['ev-character']:AddHCScore(score)

      --     local icu = exports['ev-character']:RollICU()
      --     Wait(1000)
      --     if icu then
      ...
      TriggerServerEvent('police:isDead', deathHash, deathSource, isViolent, isMinor)
...
function getDeadAnimTreeAndAnim()
  local model = GetEntityModel(PlayerPedId())

  local _, isAnimalModel = pcall(function() return exports["ev-misc-code"]:isAnimalModel(model) end)
  if not isAnimalModel then return defaultAnimTree, defaultAnim end

  local _, animTree = pcall(function() return exports["ev-misc-code"]:getAnimalModelDict(model) end)
  local _, anim = pcall(function() return exports["ev-misc-code"]:getAnimalModelAnim(model, "dead") end)
  if animTree == nil or anim == nil then return defaultAnimTree, defaultAnim end

  return animTree, anim
end
...
function DoDeathAnim(ped)
  ...
  if isMinor then
    ...
    currentAnimTree, currentAnim = getDeadAnimTreeAndAnim()
    loadAnimDict(currentAnimTree)
    TaskPlayAnim(ped, currentAnimTree, currentAnim, 8.0, -8, -1, 1, 0, 0, 0, 0)
  end
end
...
AddEventHandler('reviveFunction', function(pFromDrug)
  attemptRevive(false, pFromDrug)
end)
...
function attemptRevive(pSkipAnim, pFromDrug)
  ...
  if imDead == 1 or IsDeadAnimPlaying() or IsDeadVehAnimPlaying() then
    ...
    TriggerEvent("playerRevived", pFromDrug)
