...
local disableReduceFactor = true
...
function OnDeath(pMinor)
    ...
    local newDeaths = RPC.execute('ev-ragdoll:addDeath', pMinor)
    SetDeaths(newDeaths)
end
...
AddEventHandler('ev-spawn:characterSpawned', function()
    local deaths = RPC.execute('ev-ragdoll:fetchDeaths')
    SetDeaths(deaths)
end)

RegisterNetEvent('ev-ragdoll:refreshDeaths', function(deaths)
    SetDeaths(deaths)
end)
