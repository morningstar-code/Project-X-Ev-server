...
local function shouldSendPing()
    return (not isCop) and (not inServerRoom) and InDefaultWorld()
end
...
AddEventHandler('DoLongHudText', function(text, color, length)
    ...
    TriggerEvent("tasknotify:guiupdate",color, text, length)
end)

RegisterNetEvent('DoLHudText')
AddEventHandler('DoLHudText', function(color, text, ...)
    if HudStage > 2 then return end
    if not color then color = 1 end
