...
function plyTackel()
	if Config["cops.tackling.disabled"] and isCop then return end

	if GetLastInputMethod(2) then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), false)
		if not isInVeh and GetEntitySpeed(PlayerPedId()) > 2.5 then
			TriggerEvent("ev-police:tackle")
		end
	end
end

Citizen.CreateThread(function()
  exports["ev-keybinds"]:registerKeyMapping("", "Vehicle", "Cruise Control", "+vehCruise", "-vehCruise", "X")
	...
	if not inmenus and isCop then
		...
		if isInVeh then
			...
			if not IsControlPressed(0,19) then
				TriggerEvent("ev-police:cuffs:attempt", false)
			end
		end
	end
end
...
function policeEscort()
	if not inmenus and (isMedic or isDoctor or isCop) then
		...
		if isInVeh and isCop then
			...
			TriggerEvent("ev-police:drag:attempt") 
		end
	end
end
...
function policeSeat()
	if not inmenus and (isMedic or isCop) and not IsPauseMenuActive() then
		TriggerEvent("ev-police:vehicle:seat")
	end
end
...
function policeSoft()
	if not inmenus and isCop and not IsPauseMenuActive() then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), false)
		if not isInVeh then
			TriggerEvent("ev-police:cuffs:attemptSoft")
		end
	end
end
...
Citizen.CreateThread( function()
	...
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "Cuff / Radar Front", "+policeCuff", "-policeCuff", "UP")

	RegisterCommand('+medicRevive', medicRevive, false)
	RegisterCommand('-medicRevive', function() end, false)
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "EMS Revive", "+medicRevive", "-medicRevive")

	RegisterCommand('+emsHeal', emsHeal, false)
	RegisterCommand('-emsHeal', function() end, false)
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "EMS Heal", "+emsHeal", "-emsHeal")

	RegisterCommand('+policeEscort', policeEscort, false)
	RegisterCommand('-policeEscort', function() end, false)
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

	RegisterCommand('+policeSeat', policeSeat, false)
	RegisterCommand('-policeSeat', function() end, false)
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")

	RegisterCommand('+policeUnCuff', policeUnCuff, false)
	RegisterCommand('-policeUnCuff', function() end, false)
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "UnCuff / Radar Rear", "+policeUnCuff", "-policeUnCuff", "DOWN")

	RegisterCommand('+policeSoft', policeSoft, false)
	RegisterCommand('-policeSoft', function() end, false)
	exports["ev-keybinds"]:registerKeyMapping("", "Gov", "Soft Cuff", "+policeSoft", "-policeSoft")
end)
...
RegisterNetEvent("ev-signin:signoff")
AddEventHandler("ev-signin:signoff", function()
	isDoctor = false
	isTher = false
end)
...
AddEventHandler("police:rob", function(pArgs, pEntity)
	...
	local finished = exports["ev-taskbar"]:taskBar(15000,"Robbing",false,true,nil,false,nil,5,pEntity)
...
AddEventHandler('police:cuff2', function(t, softcuff)
	if not tryingcuff then
		...
		if(distance ~= -1 and #(GetEntityCoords(ped) - GetEntityCoords(PlayerPedId())) < 2.5 and GetEntitySpeed(ped) < 1.0) then
			if exports["isPed"]:isPed("myjob") == "police" or "doc" then
				TriggerServerEvent("police:cuffGranted2", GetPlayerServerId(t), false)
			else
				TriggerServerEvent("police:cuffGranted2", GetPlayerServerId(t), softcuff)
			end
			exports['ev-flags']:SetPedFlag(ped, 'isCuffed', true)
		else
...
--[[ RegisterNetEvent('ev-police:cuffs:attempt')
AddEventHandler('ev-police:cuffs:attempt', function(softcuff)
	if not cuffstate and not handCuffed and not IsPedRagdoll(PlayerPedId()) and not IsPlayerFreeAiming(PlayerId()) and not IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		if(distance ~= -1 and distance < 0.85 and not IsPedRagdoll(PlayerPedId())) then
			if softcuff then
...
--[[ RegisterNetEvent('ev-police:cuffs:uncuff')
AddEventHandler('ev-police:cuffs:uncuff', function()
	local t, distance, ped = GetClosestPlayer()
	if(distance ~= -1 and distance < 2) then
		...
		exports['ev-flags']:SetPedFlag(ped, 'isCuffed', false)
	else
		TriggerEvent("DoLongHudText", "No player near you (maybe get closer)!",2)
	end
end) ]]
...
AddEventHandler('resetCuffs', function()
	...
	exports['ev-flags']:SetPedFlag(PlayerPedId(), 'isCuffed', false)
end)
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	if not isDead and cuffAttemptCount < 4 then
		...
		finished = exports["ev-ui"]:taskBarSkill(1000, 15)
	end
	...
	if inRange and finished ~= 100 then
		...
		exports['ev-flags']:SetPedFlag(cuffPed, 'isCuffed', true)
...
RegisterNetEvent("ev-police:uncuffPlayer")
AddEventHandler("ev-police:uncuffPlayer", function()
	local t, distance = GetClosestPlayer()

    if distance ~= -1 and distance < 2 then
		TriggerEvent("animation:PlayAnimation", "uncuff")
		Wait(3000)
		TriggerServerEvent("ev-police:uncuff", GetPlayerServerId(t))
...
AddEventHandler('police:getArrested', function(cuffer)
	if(handCuffed) then
		...
		TriggerEvent("police:currentHandCuffedState",handCuffed,handCuffedWalking)
		TriggerEvent("handcuffed",true)
		exports['ev-flags']:SetPedFlag(cufcuffPedfer, 'isCuffed', true)
	else
		ClearPedTasksImmediately(PlayerPedId())
		CuffAnimation(cuffer)

		local cuffPed = GetPlayerPed(GetPlayerFromServerId(tonumber(cuffer)))
		if Vdist2( GetEntityCoords( PlayerPedId() , GetEntityCoords(cuffPed) ) ) < 1.5 then
			handcuffType = 49
			handCuffed = true
			TriggerEvent("police:currentHandCuffedState",handCuffed,handCuffedWalking)
			TriggerEvent("handcuffed",false)
			exports['ev-flags']:SetPedFlag(cuffPed, 'isCuffed', false)
		end
	end
end)

RegisterNetEvent('police:jailing')
AddEventHandler('police:jailing', function(pTarget, pTime)
	RPC.execute("ev-jail:seizeItems", pTarget, true)
	TriggerServerEvent('ev-jail:sendToJail', pTarget, pTime)
end)

RegisterNetEvent('police:billing')
AddEventHandler('police:billing', function(pTarget, pAmount)
	TriggerServerEvent('police:billGranted', pTarget, pAmount)
end)
...
AddEventHandler('police:forceEnter', function()
	...
	if isInVeh then
		...
		if seat then
			local finished = exports['ev-taskbar']:taskBar(7500, "Seating Player")
			if finished == 100 then
				local vehNet = NetworkGetNetworkIdFromEntity(veh)

				TriggerEvent("dr:releaseEscort")
				TriggerServerEvent("dr:releaseEscort", tServerId)
...
AddEventHandler('police:tenThirteenA', function()
  ...
  tenthirteenACooldown = GetGameTimer()
	if(isCop) then
		...
		exports["ev-dispatch"]:policedead("10-13A", "Police Injured", 189)
	end
end)
...
AddEventHandler('police:tenThirteenB', function()
  ...
  tenthirteenBCooldown = GetGameTimer()
	if(isCop) then
		...
		exports["ev-dispatch"]:policedead("10-13B", "Police Injured", 189)
	end
end)
...
AddEventHandler('requestWounds', function(pArgs, pEntity)
	local targetPed = nil
	if not pEntity then
		targetPed = exports['ev-target']:GetCurrentEntity()
	else
		targetPed = pEntity
	end 
...
Citizen.CreateThread(function()
  exports["ev-keybinds"]:registerKeyMapping("", "Gov", "(EMS) Examine Target", "+examineTarget", "-examineTarget")
end)
...
AddEventHandler("ems:bloodtest", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["ev-taskbar"]:taskBar(10000,"Taking blood test", false, true)
			if finished == 100 then
...
AddEventHandler('vehicle:swapSeat', function(num)
...
AddEventHandler('clientcheckLicensePlate', function(pDummy, pEntity)
	if isCop then
		...
		if licensePlate == nil then
			...
			TriggerServerEvent('checkLicensePlate', licensePlate, Vin)
		end
	end
end)
...
AddEventHandler('ev-police:drag:attempt', function()
	local handcuffed = exports["isPed"]:isPed("handcuffed")
	if handcuffed then
		TriggerEvent("DoLongHudText","You are in handcuffs!",2)
		return
	...
	local t, distance, ped = GetClosestPlayer()
	local checkCuffs = exports["ev-flags"]:HasPedFlag(ped, "isCuffed")
	local checkEscorting = exports["ev-flags"]:HasPedFlag(ped, "isEscorted")
	local checkIsDead = exports["ev-flags"]:HasPedFlag(ped, "isDead")

	if not checkEscorting and checkCuffs or checkIsDead then 
		if not IsPedInAnyVehicle(PlayerPedId(), false) then
			if(distance ~= -1 and distance < 5) then
				if not escort then
					TriggerEvent("dr:escortingEnabled")
					TriggerServerEvent("police:escortAsk", GetPlayerServerId(t))
					exports['ev-flags']:SetPedFlag(ped, 'isEscorted', true)
				end
			else
				escorting = false
			end
		...
		TriggerEvent("DoLongHudText","Player is not cuffed or being escorted", 2)
		return
	end
end)

RegisterNetEvent('ev-police:drag:stopForced')
AddEventHandler('ev-police:drag:stopForced', function()
	local t, distance, ped = GetClosestPlayer()
	local bonga = GetPlayerPed(tonumber(t))

	if(t and (distance ~= -1 and distance < 5)) then
		exports['ev-flags']:SetPedFlag(ped, 'isEscorted', false)
		TriggerServerEvent("dr:releaseEscort", GetPlayerServerId(t))
	end
end)
...
RegisterNetEvent("ev-police:client:fingerPrint")
AddEventHandler("ev-police:client:fingerPrint", function(pArgs, pEntity, pContext)
	TriggerServerEvent("police:fingerprint", GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity)))
end)

RegisterNetEvent("ev-police:client:seizeItems")
AddEventHandler("ev-police:client:seizeItems", function(pArgs, pEntity, pContext)
...
Citizen.CreateThread(function()
  exports["ev-polyzone"]:AddBoxZone("wheelchair_store", vector3(-803.48, -1248.7, 7.34), 1.6, 1.0, {
    heading=320,
    minZ=6.14,
    maxZ=8.74,
    data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("police_station", vector3(461.91, -993.59, 30.69), 55.8, 54.8, {
	heading=0,
	minZ = 23.49,
	maxZ = 32.29,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("police_station", vector3(-1084.8, -836.75, 13.52), 25.6, 11.2, {
	heading=309,
	minZ=12.32,
	maxZ=17.72,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("police_station", vector3(1844.54, 3685.35, 34.28), 14.4, 30.8, {
	heading=30,
	minZ=33.28,
	maxZ=37.28,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("police_station", vector3(-443.8, 6009.09, 31.71), 26.8, 9.55, {
	heading=45,
	minZ=30.71,
	maxZ=34.71,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("police_station", vector3(382.87, 796.83, 187.46), 8.0, 11.8, {
	heading=0,
	minZ=186.46,
	maxZ=193.26,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("possession_pickup", vector3(472.98, -1007.43, 26.27), 1.2, 2.6, {
	heading=0,
	minZ=25.27,
	maxZ=27.47,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("possession_pickup", vector3(-1088.5, -811.11, 19.3), 1.2, 2.6, {
	heading=40,
	minZ=18.3,
	maxZ=20.5,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("possession_pickup", vector3(384.99, 799.86, 187.46), 1.2, 2.6, {
	heading=0,
	minZ=186.46,
	maxZ=188.66,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("possession_pickup", vector3(1853.6, 3687.98, 34.28), 2.0, 1, {
	heading=120,
	minZ=33.28,
	maxZ=35.48,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("possession_pickup", vector3(-447.56, 6013.49, 31.72), 2.0, 1, {
	heading=45,
	minZ=30.72,
	maxZ=32.92,
	data = {
  ...
  exports["ev-polyzone"]:AddBoxZone("possession_pickup", vector3(1841.763, 2583.282, 46.01), 1.6, 2.6, {
	heading=0,
	minZ=45.01,
	maxZ=48.21,
	data = {
  ...
  PolyZoneInteraction("possession_pickup", "[E] Re-claim Possessions", 38, function (data)
	...
	TriggerServerEvent("ev-jail:claimPossessions", cid, false, GetPlayerServerId(PlayerId()))
    ...
    local data = {
        {
            ...
            key = true,
            children = {
				{ title = "Confirm Purchase", action = "ev-ui:wheelchairPurchase", key = true },
			},
        },
    }
...
RegisterUICallback("ev-ui:wheelchairPurchase", function(data, cb)
	local finished = exports["ev-taskbar"]:taskBar(10000, "Purchasing...", true)
	if finished ~= 100 then
...
RegisterUICallback("ev-ui:police:flagPlate", function(data, cb)
	cb({ data = {}, meta = { ok = true, message = "done" } })
...
Citizen.CreateThread(function()
	Citizen.Wait(60 * 60 * 1000)
	  local afkAutoKick = exports["ev-config"]:GetMiscConfig("afk.kick.auto")
	while true do
		Citizen.Wait(60000)
