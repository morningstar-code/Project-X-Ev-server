...
AddEventHandler('shops:vendingMachine', function (pParams, pEntity, pContext)
    if pContext then
        print(pContext.flags['isVendingMachineBeverage'])
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        --             local finished = exports["ev-taskbar"]:taskBar(60000,"Readying Smelter")
        ...
        if(Vdist(-1528.8, 846.02, 181.6, pos.x, pos.y, pos.z) < 10.0)then
            ...
            if(Vdist(-1528.8, 846.02, 181.6, pos.x, pos.y, pos.z) < 2.0)then
                DisplayHelpText("Press ~INPUT_CONTEXT~ to look at food")
                if IsControlJustPressed(1, 38) then
                    TriggerEvent("server-inventory-open", "42084", "Shop");
                    Wait(1000)
                    --TriggerEvent("openSubMenu","burgershot")
                end
            end
        ...
        --             local finished = exports["ev-taskbar"]:taskBar(60000,"Making a god slushy...")
        --               if (finished == 100) then
        ...
        --             exports["ev-ui"]:showInteraction("[E] Grab Ingredients")
        --             helpTextShowing = true
        --         end
        --         if IsControlJustPressed(1, 38) then
        --             exports["ev-ui"]:hideInteraction()
        --             local finished = exports["ev-taskbar"]:taskBar(6000, "Getting Ingredients")
        --             if finished == 100 then
        --                 pos = GetEntityCoords(PlayerPedId(), false)
        ...
        --         if not helpTextShowing then
        --             exports["ev-ui"]:showInteraction("[E] Cook Food")
        --             helpTextShowing = true
        --         end
        --         if IsControlJustPressed(1, 38) then
        --             exports["ev-ui"]:hideInteraction()
        --             local finished = exports["ev-taskbar"]:taskBar(60000, "Cooking Food")
        --             if finished == 100 then
        --                 pos = GetEntityCoords(PlayerPedId(), false)
