...
Citizen.CreateThread(function()
    ActiveAreaEvents = RPC.execute('ev:sync:getActiveEvents') or {}
    while true do
        local idle = 1000
...
RegisterNetEvent('ev:sync:active:area:start')
AddEventHandler('ev:sync:active:area:start', function (data)
    ActiveAreaEvents[#ActiveAreaEvents + 1] = data
end)

RegisterNetEvent('ev:sync:active:area:stop')
AddEventHandler('ev:sync:active:area:stop', function (pEventId)
