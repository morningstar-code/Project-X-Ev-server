...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        -- let them in the server
        local added = Queue:AddToConnecting(ids, true, true, done)
        TriggerEvent("ev-log:server:CreateLog", "JoinServer", "Queue", "orange", "Player Name :  **" .. GetPlayerName(src) .. "** (" .. json.encode(GetPlayerIdentifiers(src)) .. ") Joining The Server..")
        if not added then CancelEvent() return end
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and _Queue.JoinDelay <= GetGameTimer() then
            ...
            Queue:RemoveFromQueue(ids)
            TriggerEvent("ev-log:server:CreateLog", "JoinServer", "Queue", "orange", "Player Name :  **" .. GetPlayerName(src) .. "** (" .. GetPlayerIdentifiers(src)[1] .. ") Joining The Server..")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] is loading into the server")
            return
        end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [projectx-queue] Disabling hardcap ^7")
