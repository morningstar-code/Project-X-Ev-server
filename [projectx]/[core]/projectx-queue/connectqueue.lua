...
if Queue.CurResource == "projectx-queue" then return end

function Queue.OnReady(cb)
    if not cb then return end
    if Queue.IsReady() then cb() return end
    print(json.encode(Queue.ReadyCbs))
...
function Queue.LoadExports()
    Queue.Exports = exports['projectx-queue']:GetQueueExports()
    Queue.Ready = true
...
AddEventHandler("onResourceStart", function(resource)
    if resource == "projectx-queue" then
