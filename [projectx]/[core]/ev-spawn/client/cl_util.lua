...
function findCharPed(ped, isHover)
    ...
    if ped ~= 0 then
        ...
        if (not isHover and Login.Selected) or (not Login.Selected) then
            ...
            --for i=1,#Login.spawnLoc do --old method
...
function getCharLimit()
    local charactersLimited = 4 --exports["ev-main-config"]:GetMiscConfig("characters.limited")
    if not charactersLimited then
        return 4 --exports["ev-main-config"]:GetModuleConfig("main").characterLimit
    end

    local userData = exports["ev-base"]:getModule("LocalPlayer")
