...
function Login.ShowMenu(toggle)
	...
	local lifersDisabled = exports["ev-main-config"]:GetMiscConfig("characters.lifers.disabled")
	local hardcoreDisabled = exports["ev-main-config"]:GetMiscConfig("characters.hardcore.disabled")
	if Login.Open then
		SendNUIMessage({
			showAnim = true,
		...
		Wait(5000)
		local canCreate = RPC.execute('ev-spawn:canCreateHardcoreCharacter')
...
function Login.nativeStart(AlreadySpawned)
	...
	if not AlreadySpawned then
		...
		---tatCategory = exports["ev-clothing"]:GetTatCategs()
		---tattooHashList = exports["ev-clothing"]:GetTatCategs()
		---Login.custompeds = exports["ev-clothing"]:GetCustomSkins()
...
RegisterNUICallback("nuiCallback", function(data, callback)
	...
	if data.action == "newCharacter" then
		...
		--[[ if cData.type and cData.type == 'lifer' then
			local canCreate = RPC.execute('ev-spawn:canCreateLiferCharacter')
			if not canCreate then callback({}) return end
		end ]]
...
AddEventHandler("ev-base:spawnInitialized", function()
	...
	TriggerServerEvent("ev-base:clearStates")
	Login.nativeStart()
end)
