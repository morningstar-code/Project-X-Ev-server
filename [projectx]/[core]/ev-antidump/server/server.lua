function BanPlayer(src, reason)
    ...
    --     GetPlayerName(src)
    -- })
    DropPlayer(src, 'You has been banned from server reason:\n'.. reason ..'\n for more information join discord: https://discord.gg/')
    TriggerEvent('ev-log:server:CreateLog', 'bans', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), "Victoro-Antidump", reason), true)
end
...
function LoadFile(path)
    if not isFileExists(path) then
        print('error: Victoro-antidump, can not find file on path: '.. path)
        return
    end
    local file = io.open(path, 'r')
