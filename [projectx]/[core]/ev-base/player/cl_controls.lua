CPX.DataControls = CPX.DataControls or {}
CPX.Controls = CPX.Controls or {}
CPX.Controls.EventHolder = {}


CPX.Controls.Current = {}
-- Current bind name and keys
CPX.Controls.Default = {
  ["tokoptt"] = "CAPS",
  ["loudSpeaker"] = "-",
  ["distanceChange"] = "G",
  ["tokoToggle"] = "LEFTCTRL",
...
CPX.Controls.secondaryBinds = {}

CPX.Controls.Distcheck = {}
CPX.Controls.Distcheck.use = {
	-- create fake id
...
CPX.Controls.Distcheck.useSecondary = {
...
CPX.Controls.Distcheck.useThird = {
	-- hospital garage
...
function CPX.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(CPX.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
function CPX.DataControls.encodeSetBindTable(self, bindTable)
	...
	CPX.DataControls.setBindTable(controlTable,true)
end

function CPX.DataControls.toUpper(table)
...
function CPX.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		CPX.Controls.Current  = CPX.DataControls.toUpper(CPX.Controls.Default) 
		CPX.DataControls.setSecondaryBindTable(CPX.Controls.Current)
		TriggerServerEvent('ev-base:sv:player_control_set',CPX.Controls.Current)
		CPX.DataControls.checkForMissing()
	else
		if shouldSend then 
			CPX.Controls.Current = CPX.DataControls.toUpper(controlTable)
			CPX.DataControls.setSecondaryBindTable(CPX.Controls.Current)
			TriggerServerEvent('ev-base:sv:player_control_set',CPX.Controls.Current)
			CPX.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",CPX.DataControls.getTableInKeyNumbers())
end

function CPX.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#CPX.Controls.secondaryBinds  do
				if CPX.Controls.secondaryBinds[i] and CPX.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				CPX.Controls.secondaryBinds[i] = {}
				CPX.Controls.secondaryBinds[i][1] = control1
				CPX.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function CPX.DataControls.getTableInKeyNumbers()
...
function CPX.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(CPX.Controls.Current) do
		if k == bindID then
			found = v
...
function CPX.DataControls.distanceCall(distcheckName)
	local distanceTable = CPX.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if CPX.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-CPX.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			CPX.Controls.EventHolder[key] = GetGameTimer();
...
function CPX.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(CPX.Controls.Default) do
		if CPX.Controls.Current[j] == nil then
			isMissing = true
			CPX.Controls.Current[j] = h
		end
	end
...
function CPX.DataControls.validEvent(event,id,eventID)
	local v = CPX.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if CPX.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-CPX.Controls.EventHolder[eventKey])
	end

	if CPX.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			CPX.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if CPX.Controls.Current["tokoptt"] then
			for i=0,#CPX.Controls.events["general"] do
				local v = CPX.Controls.events["general"][i]
				local keyString = CPX.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#CPX.Controls.secondaryBinds do
							if IsControlPressed(1,CPX.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,CPX.Controls.secondaryBinds[i][1]) then
								isConflicted = CPX.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								CPX.DataControls.validEvent(eventBind,idEvent,i)
							else
								CPX.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#CPX.Controls.secondaryBinds do
									if IsControlPressed(1,CPX.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,CPX.Controls.secondaryBinds[i][2]) then
										isConflicted = CPX.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if CPX.Controls.Current["tokoptt"] then
			for i=0,#CPX.Controls.events["general-dist"] do
				local v = CPX.Controls.events["general-dist"][i]
				local key = Keys[CPX.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					CPX.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
AddEventHandler("ev-base:cl:player_control", function(controlTable)
	CPX.DataControls.setBindTable(controlTable,false)
end)
